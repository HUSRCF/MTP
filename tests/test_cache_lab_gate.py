...
def _build_payload_issue_copy_completion_blocked_canary() -> (
    PayloadCacheLiveRuntimeAdapterPayloadIssueCopyCompletionBlockedCanary
):
    return build_payload_cache_live_runtime_adapter_payload_issue_copy_completion_blocked_canary(
        _build_payload_issue_copy_descriptor_execution_blocked_canary(),
    )
...
def _build_payload_issue_residency_update_blocked_canary() -> (
    PayloadCacheLiveRuntimeAdapterPayloadIssueResidencyUpdateBlockedCanary
):
    return build_payload_cache_live_runtime_adapter_payload_issue_residency_update_blocked_canary(
        _build_payload_issue_ready_credit_blocked_canary(),
    )


def _build_payload_issue_payload_deref_blocked_canary() -> (
    PayloadCacheLiveRuntimeAdapterPayloadIssuePayloadDerefBlockedCanary
):
    return build_payload_cache_live_runtime_adapter_payload_issue_payload_deref_blocked_canary(
        _build_payload_issue_residency_update_blocked_canary(),
    )
...
def test_live_runtime_adapter_payload_issue_ready_credit_blocked_canary_consumes_completion() -> None:
    completion = _build_payload_issue_copy_completion_blocked_canary()

    canary = build_payload_cache_live_runtime_adapter_payload_issue_ready_credit_blocked_canary(
        completion,
    )
    ...
    assert canary.status == (
        f"blocked_by_payload_issue_copy_completion_blocked_canary:{completion.status}"
    )
    assert canary.consumes_payload_issue_copy_completion_blocked_canary is True
    assert canary.payload_issue_copy_completion_status == completion.status
    assert canary.payload_issue_ready_credit_schema == (
        "payload_cache_runtime_payload_issue_ready_credit_v1"
    )
    assert canary.payload_issue_ready_credit_canary_created is True
    assert canary.payload_issue_copy_completion_consumed is True
    assert canary.ready_credit_checked is True
    assert canary.ready_credit_rejected is True
    assert canary.ready_credit_allowed is False
    assert canary.ready_credit_granted is False
    assert canary.ready_before_demand_credit_granted is False
    assert canary.real_payload_ready is False
    assert canary.copy_completed is False
    assert canary.copy_descriptor_count == 0
    assert canary.copy_completion_count == 0
    assert canary.ready_credit_count == 0
    assert canary.issued_payload_count == 0
    assert canary.payload_bytes == 0
    assert canary.ready_credit is False
...
def test_live_runtime_adapter_payload_issue_ready_credit_blocked_canary_rejects_side_effects() -> None:
    completion = _build_payload_issue_copy_completion_blocked_canary()
    base_kwargs = {
        "present": True,
        "stage": "payload_cache_live_runtime_adapter_payload_issue_ready_credit_blocked_canary",
        ...
        "payload_issue_copy_completion_status": completion.status,
        ...
        "copy_descriptor_submitted": False,
        "copy_descriptor_executed": False,
        "request_source": completion.request_source,
        "request_layer_idx": completion.request_layer_idx,
        "request_expert_idx": completion.request_expert_idx,
        "requested_payload_bytes": completion.requested_payload_bytes,
        "source_issue_packet_count": completion.source_issue_packet_count,
        "source_issue_unique_key_count": completion.source_issue_unique_key_count,
        "source_queue_budget_capacity": completion.source_queue_budget_capacity,
        "source_issue_lead_tokens": completion.source_issue_lead_tokens,
        "source_queue_deadline_us": completion.source_queue_deadline_us,
    }
...
def test_live_runtime_adapter_payload_issue_ready_credit_builder_rejects_mutated_completion() -> None:
    completion = _build_payload_issue_copy_completion_blocked_canary()

    object.__setattr__(completion, "copy_completed", True)

    with pytest.raises(ValueError, match="copy_completed"):
        build_payload_cache_live_runtime_adapter_payload_issue_ready_credit_blocked_canary(
            completion,
        )

    completion = _build_payload_issue_copy_completion_blocked_canary()
    object.__setattr__(completion, "payload_issue_copy_descriptor_execution_status", "stale")
...
def test_live_runtime_adapter_payload_issue_payload_deref_blocked_canary_consumes_residency_update() -> None:
    residency = _build_payload_issue_residency_update_blocked_canary()

    canary = build_payload_cache_live_runtime_adapter_payload_issue_payload_deref_blocked_canary(
        residency,
    )
    ...
    assert canary.status == (
        f"blocked_by_payload_issue_residency_update_blocked_canary:{residency.status}"
    )
    assert canary.consumes_payload_issue_residency_update_blocked_canary is True
    assert canary.payload_issue_residency_update_status == residency.status
    assert canary.payload_issue_payload_deref_schema == (
        "payload_cache_runtime_payload_issue_payload_deref_v1"
    )
    assert canary.payload_issue_payload_deref_canary_created is True
    assert canary.payload_issue_residency_update_consumed is True
    assert canary.payload_deref_checked is True
    assert canary.payload_deref_rejected is True
    assert canary.payload_deref_allowed_for_resident_payload is False
    assert canary.payload_deref_attempted is False
    assert canary.payload_handle_deref_checked is True
    assert canary.payload_handle_deref_rejected is True
    ...
    assert canary.residency_update_count == 0
    assert canary.resident_payload_count == 0
    assert canary.payload_handle_deref_count == 0
    assert canary.resident_payload_bytes == 0
    assert canary.dereferenced_payload_bytes == 0
    assert canary.payload_deref_allowed is False
    assert canary.payload_deref_runtime_allowed is False
    assert canary.ready_credit is False
    assert canary.ready_before_demand_credit is False
    assert canary.real_ready_credit_granted is False
    assert canary.passed_to_kernel is False
...
def test_live_runtime_adapter_payload_issue_payload_deref_blocked_canary_rejects_side_effects() -> None:
    residency = _build_payload_issue_residency_update_blocked_canary()
    base_kwargs = {
        "present": True,
        "stage": (
        ...
        "payload_issue_residency_update_status": residency.status,
        ...
        "real_payload_ready": False,
        "copy_completed": False,
        "request_source": residency.request_source,
        "request_layer_idx": residency.request_layer_idx,
        "request_expert_idx": residency.request_expert_idx,
        "requested_payload_bytes": residency.requested_payload_bytes,
        "source_issue_packet_count": residency.source_issue_packet_count,
        "source_issue_unique_key_count": residency.source_issue_unique_key_count,
        "source_queue_budget_capacity": residency.source_queue_budget_capacity,
        "source_issue_lead_tokens": residency.source_issue_lead_tokens,
        "source_queue_deadline_us": residency.source_queue_deadline_us,
    }
    ...
    with pytest.raises(TypeError, match="residency"):
        build_payload_cache_live_runtime_adapter_payload_issue_payload_deref_blocked_canary(
            object(),  # type: ignore[arg-type]
        )


def test_live_runtime_adapter_payload_issue_payload_deref_builder_rejects_mutated_residency_update() -> None:
    residency = _build_payload_issue_residency_update_blocked_canary()

    object.__setattr__(residency, "payload_marked_resident", True)

    with pytest.raises(ValueError, match="payload_marked_resident"):
        build_payload_cache_live_runtime_adapter_payload_issue_payload_deref_blocked_canary(
            residency,
        )

    residency = _build_payload_issue_residency_update_blocked_canary()
    object.__setattr__(residency, "payload_issue_ready_credit_status", "stale")
...
def test_live_runtime_adapter_payload_issue_demand_hit_publication_blocked_canary_consumes_payload_deref() -> None:
    deref = _build_payload_issue_payload_deref_blocked_canary()

    canary = build_payload_cache_live_runtime_adapter_payload_issue_demand_hit_publication_blocked_canary(
        deref,
    )
    ...
    assert canary.status == (
        f"blocked_by_payload_issue_payload_deref_blocked_canary:{deref.status}"
    )
    assert canary.consumes_payload_issue_payload_deref_blocked_canary is True
    assert canary.payload_issue_payload_deref_status == deref.status
    ...
    assert canary.demand_hit_publication_allowed is False
    assert canary.demand_hit_published is False
    assert canary.consumer_visible_payload_hit is False
    assert canary.prefetched_demand_hit is False
    assert canary.payload_deref_attempted is False
    assert canary.payload_handle_deref_attempted is False
    assert canary.payload_marked_resident is False
    assert canary.resident_payload_ready is False
    ...
    assert canary.resident_payload_count == 0
    assert canary.payload_handle_deref_count == 0
    assert canary.demand_hit_publication_count == 0
    assert canary.consumer_visible_payload_hit_count == 0
    assert canary.demand_hit_count == 0
    assert canary.payload_bytes == 0
    assert canary.resident_payload_bytes == 0
    assert canary.dereferenced_payload_bytes == 0
    assert canary.demand_hit_payload_bytes == 0
    assert canary.payload_deref_allowed is False
    assert canary.payload_deref_runtime_allowed is False
    assert canary.ready_credit is False
    assert canary.ready_before_demand_credit is False
...
def test_live_runtime_adapter_payload_issue_demand_hit_publication_blocked_canary_rejects_side_effects() -> None:
    deref = _build_payload_issue_payload_deref_blocked_canary()
    base_kwargs = {
        "present": True,
        "stage": (
        ...
        "payload_issue_payload_deref_status": deref.status,
        ...
        "real_payload_ready": False,
        "copy_completed": False,
        "request_source": deref.request_source,
        "request_layer_idx": deref.request_layer_idx,
        "request_expert_idx": deref.request_expert_idx,
        "requested_payload_bytes": deref.requested_payload_bytes,
        "source_issue_packet_count": deref.source_issue_packet_count,
        "source_issue_unique_key_count": deref.source_issue_unique_key_count,
        "source_queue_budget_capacity": deref.source_queue_budget_capacity,
        "source_issue_lead_tokens": deref.source_issue_lead_tokens,
        "source_queue_deadline_us": deref.source_queue_deadline_us,
    }
    ...
    with pytest.raises(TypeError, match="deref"):
        build_payload_cache_live_runtime_adapter_payload_issue_demand_hit_publication_blocked_canary(
            object(),  # type: ignore[arg-type]
        )


def test_live_runtime_adapter_payload_issue_demand_hit_publication_builder_rejects_mutated_payload_deref() -> None:
    deref = _build_payload_issue_payload_deref_blocked_canary()

    object.__setattr__(deref, "payload_deref_attempted", True)
    ...
    deref = _build_payload_issue_payload_deref_blocked_canary()
    object.__setattr__(deref, "payload_issue_residency_update_status", "stale")
