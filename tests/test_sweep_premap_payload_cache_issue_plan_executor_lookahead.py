...
def _load_module():
    path = (
        Path(__file__).resolve().parents[1]
        / "scripts"
        / "sweep_premap_payload_cache_issue_plan_executor_lookahead.py"
    )
    spec = importlib.util.spec_from_file_location(
        "sweep_premap_payload_cache_issue_plan_executor_lookahead",
        path,
    )
    assert spec is not None
    module = importlib.util.module_from_spec(spec)
...
def test_lookahead_sweep_finds_first_model_passing_gap(tmp_path: Path):
    module = _load_module()
    ...
    result = module.run_lookahead_sweep(args)

    assert result["passed"] is True
    assert result["failures"] == []
    assert result["first_model_passing_lookahead_us"] == 3800.0
    assert [row["model_passed"] for row in result["rows"]] == [
        False,
        False,
        True,
        True,
    ]
    assert result["queue_deadline_us"] == 200.0
...
def test_lookahead_sweep_reports_no_passing_gap(tmp_path: Path):
    module = _load_module()
    gate_path = tmp_path / "gate.json"
    measured_copy_path = tmp_path / "measured_copy.json"
    _write_json(gate_path, _gate_payload())
    ...
    assert result["first_model_passing_lookahead_us"] is None
    assert all(not row["model_passed"] for row in result["rows"])


def test_lookahead_sweep_rejects_unsorted_values():
    module = _load_module()

    with pytest.raises(ValueError, match="ascending"):
        module._parse_values("1000,200,500", label="lookahead")  # noqa: SLF001
