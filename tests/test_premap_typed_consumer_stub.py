...
def test_typed_consumer_stub_uses_kernel_side_abi_header():
    ...
    assert (
        "struct PremapFutureKernelNativeConsumerLaunchEnvelopeArgsPtrV1"
        in adapter
    )
    assert (
    ...
    assert "struct PremapFutureKernelNativeConsumerEndpointV1" in adapter
    assert "context.device_ordinal == invocation.device_ordinal" in adapter
    assert "context.stream_domain == invocation.stream_domain" in adapter
    assert (
        "kPremapFutureKernelNativeConsumerLaunchEnvelopeArgsPtrAbiV1Name"
        in adapter
    )
    assert (
    ...
    assert (
        "typed_consumer_future_native_consumer_kernel_entry_args_ptr_kernel"
        in source
    )
    assert (
        "typed_consumer_future_native_consumer_launch_envelope_args_kernel"
        in source
    )
    assert (
        "typed_consumer_future_native_consumer_launch_envelope_args_ptr_kernel"
        in source
    )
    assert (
        "typed_consumer_future_native_consumer_kernel_launch_descriptor_kernel"
        in source
    )
    assert (
        "typed_consumer_future_native_consumer_kernel_launch_context_kernel"
        in source
    )
    assert "typed_consumer_future_native_consumer_invocation_kernel" in source
    assert (
        "typed_consumer_future_native_consumer_invocation_entry_kernel"
        in source
    )
    ...
    assert (
        "typed_consumer_future_wna16_typed_slot_kernel_variant_kernel"
        in source
    )
    assert (
        "typed_consumer_future_wna16_kernel_accept_typed_slot_kernel"
        in source
    )
    assert (
        "typed_consumer_future_wna16_kernel_side_consumer_execution_kernel"
        in source
    )
    assert "typed_consumer_wna16_side_consumer_variant_execution_kernel" in source
    assert (
        "PremapFutureKernelNativeConsumerWna16AdjacentTypedSlotV1" in adapter
    )
    assert "PremapFutureWna16TypedSlotKernelVariantV1" in adapter
    assert "PremapFutureWna16KernelAcceptTypedSlotV1" in adapter
    assert "PremapFutureWna16KernelSideConsumerExecutionV1" in adapter
    assert "PremapWna16SideConsumerVariantExecutionV1" in adapter
    assert (
    ...
    assert "future_kernel_native_consumer_endpoint_checked" in source
    ...
    assert "future_wna16_typed_slot_kernel_variant_checked" in source
    assert "future_wna16_typed_slot_kernel_variant_summary_error_count" in source
    assert "future_wna16_typed_slot_kernel_variant_current_wna16_arg_compatible" in source
    assert "future_wna16_typed_slot_kernel_variant_requires_wna16_arg_reinterpretation" in source
    assert "future_wna16_typed_slot_kernel_variant_reuses_current_wna16_arg_slot" in source
    assert "future_wna16_typed_slot_kernel_variant_error_count == 0" in source
    assert "future_wna16_kernel_accept_typed_slot_checked" in source
    assert "future_wna16_kernel_accept_typed_slot_summary_error_count" in source
    assert "future_wna16_kernel_accept_typed_slot_current_wna16_arg_compatible" in source
    assert (
        "future_wna16_kernel_accept_typed_slot_requires_wna16_arg_reinterpretation"
        in source
    )
    assert "future_wna16_kernel_accept_typed_slot_reuses_current_wna16_arg_slot" in source
    assert "future_wna16_kernel_accept_typed_slot_error_count == 0" in source
    assert "future_wna16_kernel_side_consumer_execution_checked" in source
    assert "future_wna16_kernel_side_consumer_execution_row_ok_count" in source
    assert (
    ...
    assert "future_wna16_kernel_side_consumer_execution_error_count == 0" in source
    assert "PremapFutureWna16SingleFieldHandoffCanaryV1" in adapter
    assert (
        "premap_typed_consumer_future_wna16_single_field_handoff_canary_matches_v1"
        in adapter
    )
    assert "future_wna16_single_field_handoff_canary_checked" in source
    assert "future_wna16_single_field_handoff_canary_field_name" in source
    assert "future_wna16_single_field_handoff_canary_row_ok_count" in source
    assert "future_wna16_single_field_handoff_canary_live_enabled" in source
    assert "future_wna16_single_field_handoff_canary_passed_to_kernel" in source
    assert "wna16_side_consumer_variant_execution_checked" in source
    assert "wna16_side_consumer_variant_execution_row_ok_count" in source
    assert "wna16_side_consumer_variant_execution_descriptor_ptr_read_row_ok_count" in source
    assert (
    ...
    assert "wna16_side_consumer_variant_execution_current_wna16_arg_compatible" in source
    assert (
        "wna16_side_consumer_variant_execution_requires_wna16_arg_reinterpretation"
        in source
    )
    assert "wna16_side_consumer_variant_execution_reuses_current_wna16_arg_slot" in source
    assert "wna16_side_consumer_variant_execution_error_count == 0" in source
    ...
    assert "future_kernel_native_consumer_view_row_struct_size" in source
    assert "future_kernel_native_consumer_view_row_offset_descriptor_ptr" in source
    ...
    assert "future_kernel_native_consumer_invocation_field_read_path" in source
    assert (
        "future_kernel_native_consumer_invocation_offset_context_struct_size"
        in source
    )
    assert "future_kernel_native_consumer_invocation_summary_error_count" in source
    assert (
...
def test_typed_consumer_stub_dry_run_accepts_future_native_program_view_ptr_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_program_view_ptr.json"
...
def test_typed_consumer_stub_rejects_future_native_program_view_ptr_without_program_view():
    module = _load_module()

    with pytest.raises(
        ValueError, match="future native consumer program-view pointer ABI requires"
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_native_entry_args_ptr_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_entry_args_ptr.json"
...
def test_typed_consumer_stub_rejects_future_native_entry_args_ptr_without_entry_args():
    module = _load_module()

    with pytest.raises(
        ValueError, match="future native consumer kernel entry-args pointer ABI requires"
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_native_launch_envelope_args_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_launch_envelope_args.json"
...
def test_typed_consumer_stub_rejects_future_native_launch_envelope_args_without_entry_args_ptr():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="future native consumer launch-envelope args ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_native_launch_envelope_args_ptr_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_launch_envelope_args_ptr.json"
...
def test_typed_consumer_stub_rejects_future_native_launch_envelope_args_ptr_without_launch_envelope_args():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="future native consumer launch-envelope args pointer ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_native_kernel_launch_descriptor_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_kernel_launch_descriptor.json"
...
def test_typed_consumer_stub_rejects_future_native_kernel_launch_descriptor_without_launch_envelope_args_ptr():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="future native consumer kernel launch descriptor ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_native_kernel_launch_context_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_kernel_launch_context.json"
...
def test_typed_consumer_stub_rejects_future_native_kernel_launch_context_without_descriptor():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="future native consumer kernel launch context ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_native_invocation_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_invocation.json"
...
def test_typed_consumer_stub_rejects_future_native_invocation_without_context():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="future native consumer invocation ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_records_kernel_launch_descriptor_fault_hook(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_kernel_launch_descriptor_fault.json"
    ...
    payload = json.loads(output.read_text(encoding="utf-8"))
    assert exit_code == 0
    assert payload["fault_kernel_launch_descriptor_schema_hash"] is True


def test_typed_consumer_stub_dry_run_records_invocation_device_fault_hook(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_invocation_device_fault.json"
    ...
    payload = json.loads(output.read_text(encoding="utf-8"))
    assert exit_code == 0
    assert payload["fault_invocation_device_ordinal"] is True


def test_typed_consumer_stub_dry_run_records_invocation_stream_fault_hook(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_invocation_stream_fault.json"
    ...
    payload = json.loads(output.read_text(encoding="utf-8"))
    assert exit_code == 0
    assert payload["fault_invocation_stream_domain"] is True


def test_typed_consumer_stub_dry_run_accepts_invocation_entry_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_invocation_entry.json"
...
def test_typed_consumer_stub_rejects_invocation_entry_without_invocation_abi():
    module = _load_module()

    with pytest.raises(
        ValueError, match="future native consumer invocation entry ABI requires"
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_endpoint_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_endpoint.json"
...
def test_typed_consumer_stub_dry_run_accepts_endpoint_ptr_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_endpoint_ptr.json"
...
def test_typed_consumer_stub_rejects_endpoint_without_invocation_entry_abi():
    module = _load_module()

    with pytest.raises(
        ValueError, match="future native consumer endpoint ABI requires"
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_rejects_endpoint_ptr_without_endpoint_abi():
    module = _load_module()

    with pytest.raises(
        ValueError, match="future native consumer endpoint pointer ABI requires"
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_wna16_typed_slot_variant_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_wna16_typed_slot_variant.json"
    macros = [
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_LAUNCH_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_DISPATCH_ABI",
    ...
    assert any(
        "-DMTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_TYPED_SLOT_KERNEL_VARIANT_ABI=1"
        == part
        for part in payload["build_command"]
    )


def test_typed_consumer_stub_rejects_future_wna16_variant_without_adjacent_slot():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="future WNA16 typed-slot kernel variant ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_wna16_accept_typed_slot_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_wna16_accept_typed_slot.json"
    macros = [
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_LAUNCH_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_DISPATCH_ABI",
    ...
    assert any(
        "-DMTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_KERNEL_ACCEPT_TYPED_SLOT_ABI=1"
        == part
        for part in payload["build_command"]
    )


def test_typed_consumer_stub_rejects_future_wna16_accept_without_adjacent_slot():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="future WNA16 kernel accept-typed-slot ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_wna16_kernel_side_execution_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_wna16_kernel_side_execution.json"
    macros = [
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_LAUNCH_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_DISPATCH_ABI",
    ...
    assert any(
        "-DMTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_KERNEL_SIDE_CONSUMER_EXECUTION_ABI=1"
        == part
        for part in payload["build_command"]
    )


def test_typed_consumer_stub_rejects_future_wna16_kernel_side_execution_without_accept():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="future WNA16 kernel-side consumer execution ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_future_wna16_single_field_handoff_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_wna16_single_field_handoff.json"
    macros = [
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_LAUNCH_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_DISPATCH_ABI",
    ...
    assert any(
        "-DMTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_SINGLE_FIELD_HANDOFF_CANARY_ABI=1"
        == part
        for part in payload["build_command"]
    )


def test_typed_consumer_stub_rejects_future_wna16_single_field_handoff_without_execution():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="future WNA16 single-field handoff canary ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_SINGLE_FIELD_HANDOFF_CANARY_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_wna16_side_execution_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_wna16_side_execution.json"
    macros = [
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_LAUNCH_ABI",
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_DISPATCH_ABI",
    ...
    assert any(
        "-DMTP_PREMAP_TYPED_CONSUMER_CHECK_WNA16_SIDE_CONSUMER_VARIANT_EXECUTION_ABI=1"
        == part
        for part in payload["build_command"]
    )


def test_typed_consumer_stub_rejects_wna16_side_execution_without_future_variant():
    module = _load_module()

    with pytest.raises(
        ValueError,
        match="WNA16-side consumer variant execution ABI requires",
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_request_ptr_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_request_ptr.json"
...
def test_typed_consumer_stub_rejects_request_ptr_without_kernel_arg_packet_abi():
    module = _load_module()

    with pytest.raises(
        ValueError, match="future native consumer request pointer ABI requires"
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_request_launch_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_request_launch.json"
...
def test_typed_consumer_stub_rejects_request_launch_without_request_ptr_abi():
    module = _load_module()

    with pytest.raises(
        ValueError, match="future native consumer request launch ABI requires"
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_dry_run_accepts_request_launch_ptr_macro(
    tmp_path: Path,
) -> None:
    module = _load_module()
    output = tmp_path / "dry_run_future_native_request_launch_ptr.json"
...
def test_typed_consumer_stub_rejects_request_launch_ptr_without_request_launch_abi():
    module = _load_module()

    with pytest.raises(
        ValueError, match="future native consumer request launch pointer ABI requires"
    ):
        module.validate_macros(
            [
                "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ABI",
...
def test_typed_consumer_stub_returns_structured_payload_for_fault_non_json(
    ...
    monkeypatch.setattr(module, "build", lambda **_: tmp_path / "fake_stub")
    ...
    def fake_run_cmd(cmd, *, env=None, allow_failure=False):
        ...
        return module.subprocess.CompletedProcess(
            ...
            2,
            stdout="not-json",
            stderr="native failed before JSON",
        )

    monkeypatch.setattr(module, "run_cmd", fake_run_cmd)

    payload = module.run_stub(args)

    assert payload["ok"] is False
    assert payload["passed"] is False
    assert payload["native_returncode"] == 2
    assert payload["fault_invocation_stream_domain"] is True
    assert "native_json_parse_error" in payload
    assert payload["native_stdout"] == "not-json"
    assert payload["stderr"] == "native failed before JSON"
