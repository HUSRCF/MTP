...
def test_kernel_consumer_schema_accepts_valid_artifact(tmp_path: Path) -> None:
    ...
    assert result["future_kernel_consumer_args_layout_reported"] is True
    assert (
        result["future_kernel_consumer_args_layout_fields"]
        == FUTURE_KERNEL_CONSUMER_ARGS_LAYOUT_FIELDS
    )
    assert (
        result["future_kernel_consumer_args_layout_expected"]
        == FUTURE_KERNEL_CONSUMER_ARGS_LAYOUT_EXPECTED
    )
    ...
    assert (
        result["future_kernel_native_consumer_arg_slot_abi_name"]
        == "premap_future_kernel_native_consumer_arg_slot_abi_v1"
    )
    assert (
        result["future_kernel_native_consumer_arg_slot_abi_mode"]
        == "readonly_future_kernel_native_consumer_arg_slot_abi"
    )
    assert (
        result["future_kernel_native_consumer_arg_slot_abi_current_wna16_arg_compatible"]
        is False
    )
    assert (
        result["future_kernel_native_consumer_arg_slot_abi_layout_fields"]
        == FUTURE_KERNEL_NATIVE_CONSUMER_ARG_SLOT_ABI_LAYOUT_FIELDS
    )
    assert (
    ...
    assert (
        result["future_kernel_native_consumer_kernel_arg_packet_abi_name"]
        == "premap_future_kernel_native_consumer_kernel_arg_packet_abi_v1"
    )
    assert (
        result["future_kernel_native_consumer_kernel_arg_packet_abi_mode"]
        == "readonly_future_kernel_native_consumer_kernel_arg_packet_abi"
    )
    assert (
        result["future_kernel_native_consumer_kernel_arg_packet_abi_source"]
        == "premap_future_kernel_native_consumer_program_view_ptr_abi_v1"
    )
    assert (
        result[
    ...
    assert (
        result["future_kernel_native_consumer_kernel_arg_packet_abi_layout_fields"]
        == FUTURE_KERNEL_NATIVE_CONSUMER_KERNEL_ARG_PACKET_ABI_LAYOUT_FIELDS
    )
    assert (
        result["future_kernel_native_consumer_kernel_arg_packet_abi_layout_expected"]
        == FUTURE_KERNEL_NATIVE_CONSUMER_KERNEL_ARG_PACKET_ABI_LAYOUT_EXPECTED
    )
    assert (
        result["future_kernel_native_consumer_kernel_entry_summary_abi_layout_fields"]
        == FUTURE_KERNEL_NATIVE_CONSUMER_KERNEL_ENTRY_SUMMARY_ABI_LAYOUT_FIELDS
    )
    assert (
        result["future_kernel_native_consumer_kernel_entry_summary_abi_layout_expected"]
        == FUTURE_KERNEL_NATIVE_CONSUMER_KERNEL_ENTRY_SUMMARY_ABI_LAYOUT_EXPECTED
    )
    assert (
        result["future_kernel_native_consumer_kernel_entry_args_abi_layout_fields"]
        == FUTURE_KERNEL_NATIVE_CONSUMER_KERNEL_ENTRY_ARGS_ABI_LAYOUT_FIELDS
    )
    assert (
...
def test_kernel_consumer_schema_rejects_missing_future_args_layout_contract(
    tmp_path: Path,
) -> None:
    payload = _valid_schema_payload()
    payload["native_consumer_abi"].pop("future_kernel_consumer_args_layout_reported")
    schema_path = tmp_path / "schema.yaml"
    _write_schema(schema_path, payload)
...
def test_kernel_consumer_schema_rejects_future_args_layout_value_drift(
    tmp_path: Path,
) -> None:
    payload = _valid_schema_payload()
    payload["native_consumer_abi"]["future_kernel_consumer_args_layout_expected"][
    ...
    assert any(
        failure.startswith(
            "native_consumer_abi."
            "future_kernel_consumer_args_layout_expected_mismatch"
        )
        for failure in result["failures"]
    )


def test_kernel_consumer_schema_rejects_arg_slot_layout_value_drift(
    tmp_path: Path,
) -> None:
    payload = _valid_schema_payload()
    payload["native_consumer_abi"][
        "future_kernel_native_consumer_arg_slot_abi_layout_expected"
    ]["future_kernel_native_arg_slot_consumer_slot_struct_size"] = 40
    schema_path = tmp_path / "schema.yaml"
    _write_schema(schema_path, payload)
