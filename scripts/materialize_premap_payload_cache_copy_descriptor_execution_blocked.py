#!/usr/bin/env python3
"""Build a payload-copy execution-blocked artifact from dispatch-blocked rows."""
...
def build_execution_blocked_artifact(
    *,
    copy_descriptor_dispatch_blocked_json: str | Path,
    output_json: str | Path,
    execution_capacity: int,
    require_pass: bool = False,
) -> dict[str, Any]:
    source_path = _resolve(copy_descriptor_dispatch_blocked_json)
    ...
    dispatch_queue_planned_payload_bytes = _int_metric(
        source,
        "dispatch_queue_planned_payload_bytes",
    )
    ...
    if (
        dispatch_queue_row_count is None
        or descriptor_count is None
        or dispatch_queue_row_count != descriptor_count
    ):
        failures.append("source_dispatch_queue_row_count_mismatch")
    if (
        dispatch_capacity is None
        or descriptor_count is None
        or dispatch_capacity < descriptor_count
    ):
        failures.append("source_dispatch_capacity_invalid")
    ...
    if (
        ...
        or dispatch_queue_planned_payload_bytes is None
        or planned_payload_bytes != dispatch_queue_planned_payload_bytes
        or descriptor_count is None
    ...
    execution_capacity_value = int(execution_capacity)
    if execution_capacity_value <= 0:
        failures.append("execution_capacity_invalid")
    if descriptor_count is not None and execution_capacity_value < descriptor_count:
        failures.append("execution_capacity_too_small")

    execution_queue_row_count = int(descriptor_count or 0)
    execution_queue_planned_payload_bytes = int(planned_payload_bytes or 0)
    passed = not failures
    payload = {
        ...
        "passed": passed,
        "failures": failures,
        "copy_descriptor_execution_blocked_ready": passed,
        "source_copy_descriptor_dispatch_blocked_json": str(source_path),
        "source_copy_descriptor_dispatch_blocked_sha256": _sha256(source_path),
        ...
        "copy_descriptor_count": execution_queue_row_count,
        ...
        "planned_payload_bytes": execution_queue_planned_payload_bytes,
        ...
        "execution_queue_shape_checked": passed,
        "execution_capacity": execution_capacity_value,
        "execution_queue_row_count": execution_queue_row_count,
        "execution_queue_planned_payload_bytes": execution_queue_planned_payload_bytes,
        ...
        "live_runtime_instantiated": False,
        "requires_payload_runtime": False,
        "execution_mode": "payload_cache_copy_descriptor_execution_blocked",
        "next_runtime_stage": "payload_copy_completion_blocked_or_native_execution_adapter",
    }
    ...
    if require_pass and not passed:
        raise SystemExit(f"execution-blocked artifact failed validation: {failures}")
    return payload


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--copy-descriptor-dispatch-blocked-json", required=True)
    parser.add_argument("--output-json", required=True)
    parser.add_argument("--execution-capacity", type=int, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = build_execution_blocked_artifact(
        copy_descriptor_dispatch_blocked_json=args.copy_descriptor_dispatch_blocked_json,
