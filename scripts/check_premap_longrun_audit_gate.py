...
def _normalize_summary(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(aggregate, dict):
        flat_aggregate_prefix = "runtime_shadow_aggregate_"
        aggregate = {
            str(key)[len(flat_aggregate_prefix) :]: value
            for key, value in summary.items()
            if str(key).startswith(flat_aggregate_prefix)
        }
    ...
    for key, value in summary.items():
        ...
        if key_str.startswith(
            "runtime_shadow_premap_kernel_arg_live_mutation_"
        ) or key_str.startswith("runtime_shadow_premap_kernel_arg_handoff_"):
...
def check_summary(
    ...
    *,
    ...
    allow_single_field_replacement_wrapper_only_canary: bool = False,
    expected_single_field_replacement_allow_signature_mismatch_live: (
        bool | None
    ) = None,
) -> dict[str, Any]:
    ...
    if (
        allow_single_field_replacement_wrapper_only_canary
        and not allow_single_field_replacement_live
    ):
        failures.append(
            "allow_single_field_replacement_wrapper_only_canary_requires_single_field_live"
        )
    if (
    ...
    if expected_single_field_replacement_allow_signature_mismatch_live is not None:
        observed_allow_signature_mismatch = _as_bool(
            single_field_replacement_allow_signature_mismatch_live
        )
        if (
            observed_allow_signature_mismatch
            != expected_single_field_replacement_allow_signature_mismatch_live
        ):
            failures.append(
                "single_field_replacement_allow_signature_mismatch_live_mismatch="
                f"{observed_allow_signature_mismatch}!="
                f"{expected_single_field_replacement_allow_signature_mismatch_live}"
            )
    ...
    required_event_types = (
        {"premap_summary"}
        if allow_single_field_replacement_wrapper_only_canary
        else REQUIRED_EVENT_TYPES
    )
    missing_events = sorted(
        event for event in required_event_types if event_counts.get(event, 0) <= 0
    )
    if unknown_events:
    ...
    if (
        premap_count != consumer_count
        and not allow_single_field_replacement_wrapper_only_canary
    ):
        failures.append(
    ...
    if not allow_single_field_replacement_wrapper_only_canary:
    ...
    if (
        (require_readonly_consumer or readonly_fields_present)
        and not allow_single_field_replacement_wrapper_only_canary
    ):
    ...
    if (
        ...
        failures.append(
            "consumer_shim_kernel_side_consumer_schema_adapter_fields_missing"
        )
    if (
        (
            require_kernel_side_consumer_schema_adapter
            or kernel_side_consumer_schema_adapter_active
        )
        and not kernel_arg_semantic_handle_adapter_active
    ):
        failures.append(
    ...
    if (
        ...
        if (
            ...
            if (
                ...
                if single_field_handle_handoff_canary_active:
                    required_canary_metric_suffixes = (
                        "checked_count",
                        "ready_count",
                        "hash_checked_count",
                        "hash_missing_count",
                        ...
                        "mirror_mode",
                        "mirror_mode_checked_count",
                        "mirror_mode_missing_count",
                        "mirror_mode_mismatch_count",
                        "mirror_ready_count",
                        "mirror_field_name",
                        "mirror_field_name_checked_count",
                        "mirror_field_name_missing_count",
                        "mirror_field_name_mismatch_count",
                        "mirror_source",
                        "mirror_source_checked_count",
                        "mirror_source_missing_count",
                        "mirror_source_mismatch_count",
                        "block_reason",
                        "block_reason_checked_count",
                        ...
                        "kernel_side_typed_consumer_compatible_count",
                        "current_wna16_arg_compatible_count",
                    )
                    for suffix in required_canary_metric_suffixes:
                        if f"{canary_prefix}{suffix}" not in aggregate:
                            failures.append(
                                "consumer_shim_single_field_handle_handoff_canary_"
                                f"{suffix}_field_missing"
                            )
            ...
            if (
                ...
                kernel_side_prefix = KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX
                kernel_side_checked_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}checked_count")
                )
                kernel_side_ready_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}ready_count")
                )
                kernel_side_mode = str(aggregate.get(f"{kernel_side_prefix}mode") or "")
                kernel_side_mode_checked_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}mode_checked_count")
                )
                kernel_side_mode_missing_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}mode_missing_count")
                )
                kernel_side_mode_mismatch_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}mode_mismatch_count")
                )
                kernel_side_row_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}row_count")
                )
                kernel_side_column_count_max = _as_int(
                    aggregate.get(f"{kernel_side_prefix}column_count_max")
                )
                kernel_side_column_count_min = _as_int(
                    aggregate.get(f"{kernel_side_prefix}column_count_min")
                )
                kernel_side_table_schema_hash = str(
                    aggregate.get(f"{kernel_side_prefix}table_schema_hash") or ""
                )
                kernel_side_semantic_schema_hash = str(
                    aggregate.get(f"{kernel_side_prefix}semantic_schema_hash") or ""
                )
                kernel_side_schema_name = str(
                    aggregate.get(f"{kernel_side_prefix}kernel_side_schema_name") or ""
                )
                kernel_side_schema_hash = str(
                    aggregate.get(f"{kernel_side_prefix}kernel_side_schema_hash") or ""
                )
                kernel_side_field_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}kernel_side_field_count")
                )
                kernel_side_required_source_hit_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}required_source_hit_count")
                )
                kernel_side_required_source_miss_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}required_source_miss_count")
                )
                kernel_side_optional_source_hit_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}optional_source_hit_count")
                )
                kernel_side_optional_source_miss_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}optional_source_miss_count")
                )
                kernel_side_handle_field_read_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}handle_field_read_count")
                )
                kernel_side_consumer_schema_present_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}consumer_schema_present_count")
                )
                kernel_side_consumer_connected_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}consumer_connected_count")
                )
                kernel_side_live_enabled_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}live_enabled_count")
                )
                kernel_side_live_eligible_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}live_eligible_count")
                )
                kernel_side_blocked_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}blocked_count")
                )
                kernel_side_block_reason = str(
                    aggregate.get(f"{kernel_side_prefix}block_reason") or ""
                )
                kernel_side_payload_bytes = _as_int(
                    aggregate.get(f"{kernel_side_prefix}payload_bytes")
                )
                kernel_side_payload_violation_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}payload_violation_count")
                )
                kernel_side_passed_to_kernel_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}passed_to_kernel_count")
                )
                kernel_side_kernel_arg_violation_count = _as_int(
                    aggregate.get(f"{kernel_side_prefix}kernel_arg_violation_count")
                )
                kernel_side_current_wna16_compatible_count = _as_int(
                    aggregate.get(
                        f"{kernel_side_prefix}live_compatible_with_current_wna16_args_count"
                    )
                )
                for name in (
                    "hash",
                    ...
                    checked = _as_int(
                        aggregate.get(f"{kernel_side_prefix}{name}_checked_count")
                    )
                    missing = _as_int(
                        aggregate.get(f"{kernel_side_prefix}{name}_missing_count")
                    )
                    if checked != shim_executed:
                        failures.append(
                ...
                for name in (
                    ...
                    mismatch = _as_int(
                        aggregate.get(f"{kernel_side_prefix}{name}_mismatch_count")
                    )
                    if missing != 0:
                        failures.append(
                            "consumer_shim_kernel_side_consumer_schema_adapter_"
                ...
                if kernel_side_checked_count != shim_executed:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_checked_count_mismatch="
                        f"{kernel_side_checked_count}!={shim_executed}"
                    )
                if kernel_side_ready_count != shim_executed:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_ready_count_mismatch="
                        f"{kernel_side_ready_count}!={shim_executed}"
                    )
                if (
                    kernel_side_mode
                    ...
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_mode_mismatch"
                    )
                if kernel_side_mode_checked_count != shim_executed:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_mode_checked_count_mismatch="
                        f"{kernel_side_mode_checked_count}!={shim_executed}"
                    )
                if kernel_side_mode_missing_count != 0:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_mode_missing_count_nonzero="
                        f"{kernel_side_mode_missing_count}"
                    )
                if kernel_side_mode_mismatch_count != 0:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_mode_mismatch_count_nonzero="
                        f"{kernel_side_mode_mismatch_count}"
                    )
                if kernel_side_row_count != consume_row_count:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_row_count_mismatch="
                        f"{kernel_side_row_count}!={consume_row_count}"
                    )
                if (
                    kernel_side_column_count_max
                    != EXPECTED_KERNEL_ARG_SHADOW_TABLE_COLUMN_COUNT
                ):
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_column_count_max_mismatch="
                        f"{kernel_side_column_count_max}!={EXPECTED_KERNEL_ARG_SHADOW_TABLE_COLUMN_COUNT}"
                    )
                if (
                    kernel_side_column_count_min
                    != EXPECTED_KERNEL_ARG_SHADOW_TABLE_COLUMN_COUNT
                ):
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_column_count_min_mismatch="
                        f"{kernel_side_column_count_min}!={EXPECTED_KERNEL_ARG_SHADOW_TABLE_COLUMN_COUNT}"
                    )
                if (
                    kernel_side_table_schema_hash
                    != PREMAP_DESCRIPTOR_CONSUMER_HANDLE_TABLE_SCHEMA_HASH
                ):
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_table_schema_hash_mismatch"
                    )
                if (
                    kernel_side_semantic_schema_hash
                    != PREMAP_KERNEL_ARG_SEMANTIC_HANDLE_SCHEMA_HASH
                ):
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_semantic_schema_hash_mismatch"
                    )
                if kernel_side_schema_name != PREMAP_KERNEL_SIDE_CONSUMER_SCHEMA_NAME:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_schema_name_mismatch"
                    )
                if kernel_side_schema_hash != PREMAP_KERNEL_SIDE_CONSUMER_SCHEMA_HASH:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_schema_hash_mismatch"
                    )
                expected_kernel_side_field_count = (
                    shim_executed * len(PREMAP_KERNEL_SIDE_CONSUMER_SCHEMA_FIELDS)
                )
                if kernel_side_field_count != expected_kernel_side_field_count:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_field_count_mismatch="
                        f"{kernel_side_field_count}!={expected_kernel_side_field_count}"
                    )
                if (
                    kernel_side_required_source_hit_count
                    != expected_consume_required_fields
                ):
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_required_source_hit_count_mismatch="
                        f"{kernel_side_required_source_hit_count}!={expected_consume_required_fields}"
                    )
                if kernel_side_required_source_miss_count != 0:
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_required_source_miss_count_nonzero="
                        f"{kernel_side_required_source_miss_count}"
                    )
                if (
                    kernel_side_optional_source_hit_count
                    + kernel_side_optional_source_miss_count
                    != consume_row_count
                ):
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_optional_source_total_mismatch="
                        f"{kernel_side_optional_source_hit_count}+"
                        f"{kernel_side_optional_source_miss_count}!={consume_row_count}"
                    )
                if (
                    kernel_side_handle_field_read_count
                    != consume_row_count * EXPECTED_KERNEL_ARG_SHADOW_TABLE_COLUMN_COUNT
                ):
                    failures.append(
                        "consumer_shim_kernel_side_consumer_schema_adapter_handle_field_read_count_mismatch="
                        f"{kernel_side_handle_field_read_count}!="
                        f"{consume_row_count * EXPECTED_KERNEL_ARG_SHADOW_TABLE_COLUMN_COUNT}"
                    )
                if kernel_side_consumer_schema_present_count != shim_executed:
                    failures.append(
                ...
                for name, value in (
                    ...
                    ("kernel_arg_violation_count", kernel_side_kernel_arg_violation_count),
                    (
                        "current_wna16_compatible_count",
                        kernel_side_current_wna16_compatible_count,
                    ),
                ):
                    if value != 0:
                        failures.append(
                ...
                if kernel_side_blocked_count != shim_executed:
                    failures.append(
    ...
    if _as_bool(single_field_replacement_dry_run_enabled):
        ...
        if _as_bool(single_field_replacement_live_enabled):
            ...
            if single_field_replacement_candidate_source == "prepared_handle_table":
                if single_field_replacement_live_replaced_count != 0:
                    failures.append(
                        "single_field_replacement_live_prepared_table_replaced_count_nonzero="
                        f"{single_field_replacement_live_replaced_count}"
                    )
                if single_field_replacement_live_parity_ok_count != 0:
                    failures.append(
                        "single_field_replacement_live_prepared_table_parity_ok_nonzero="
                        f"{single_field_replacement_live_parity_ok_count}"
                    )
                if single_field_replacement_live_passed_to_kernel_count != 0:
                    failures.append(
                        "single_field_replacement_live_prepared_table_passed_to_kernel_nonzero="
                ...
                if (
                    ...
                    failures.append(
                        "single_field_replacement_live_prepared_table_parity_mismatch_count_mismatch="
                ...
                if (
                    ...
                    failures.append(
                        "single_field_replacement_live_prepared_table_type_mismatch_fallback_count_mismatch="
                        f"{single_field_replacement_live_type_mismatch_fallback_count}!="
                        f"{single_field_replacement_live_candidate_count}"
                    )
                if single_field_replacement_live_source_missing_fallback_count != 0:
                    failures.append(
                        "single_field_replacement_live_prepared_table_source_missing_fallback_nonzero="
                        f"{single_field_replacement_live_source_missing_fallback_count}"
                    )
                if single_field_replacement_live_signature_mismatch_allowed_count != 0:
                    failures.append(
                        "single_field_replacement_live_prepared_table_signature_mismatch_allowed_nonzero="
                        f"{single_field_replacement_live_signature_mismatch_allowed_count}"
                    )
                if single_field_replacement_live_signature_mismatch_blocked_count != 0:
                    failures.append(
                        "single_field_replacement_live_prepared_table_signature_mismatch_blocked_nonzero="
                        f"{single_field_replacement_live_signature_mismatch_blocked_count}"
                    )
            else:
                if (
                ...
                for name, count in (
                    (
                        "source_missing_fallback",
                        single_field_replacement_live_source_missing_fallback_count,
                    ),
                    (
                        "type_mismatch_fallback",
                        single_field_replacement_live_type_mismatch_fallback_count,
                    ),
                    (
                        "signature_mismatch_allowed",
                        single_field_replacement_live_signature_mismatch_allowed_count,
                    ),
                    (
                    ...
                    if count != 0:
                        failures.append(
                            f"single_field_replacement_live_{name}_count_nonzero={count}"
                        )
    ...
    metrics = {
        ...
        "runtime_shadow_premap_kernel_arg_handoff_single_field_replacement_allow_signature_mismatch_live": _as_bool(
            single_field_replacement_allow_signature_mismatch_live
        ),
        "runtime_shadow_premap_kernel_arg_live_mutation_package_seen_count": (
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_hash_missing_count": _as_int(
            aggregate.get(f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}hash_missing_count")
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_table_object_hash_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}table_object_hash_missing_count"
            )
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_semantic_adapter_hash_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}semantic_adapter_hash_missing_count"
            )
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_field_handle_hash_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}field_handle_hash_missing_count"
            )
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_semantic_field_hash_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}semantic_field_hash_missing_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_handle_hash_checked_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_handle_hash_checked_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_handle_hash_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_handle_hash_missing_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_schema_hash_checked_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_schema_hash_checked_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_schema_hash_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_schema_hash_missing_count"
            )
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mode_checked_count": _as_int(
            aggregate.get(f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mode_checked_count")
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mode_missing_count": _as_int(
            aggregate.get(f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mode_missing_count")
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mode_mismatch_count": _as_int(
            aggregate.get(f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mode_mismatch_count")
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_field_name_checked_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}field_name_checked_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_field_name_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}field_name_missing_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_field_name_mismatch_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}field_name_mismatch_count"
            )
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_source_checked_count": _as_int(
            aggregate.get(f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}source_checked_count")
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_source_missing_count": _as_int(
            aggregate.get(f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}source_missing_count")
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_source_mismatch_count": _as_int(
            aggregate.get(f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}source_mismatch_count")
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_mode_checked_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_mode_checked_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_mode_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_mode_missing_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_mode_mismatch_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_mode_mismatch_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_ready_count": _as_int(
            aggregate.get(f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_ready_count")
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_field_name_checked_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_field_name_checked_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_field_name_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_field_name_missing_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_field_name_mismatch_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_field_name_mismatch_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_source": str(
            aggregate.get(f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_source")
            or ""
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_source_checked_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_source_checked_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_source_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_source_missing_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_mirror_source_mismatch_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}mirror_source_mismatch_count"
            )
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_block_reason_checked_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}block_reason_checked_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_block_reason_missing_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}block_reason_missing_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_block_reason_mismatch_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}block_reason_mismatch_count"
            )
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_payload_violation_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}payload_violation_count"
            )
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_kernel_side_typed_consumer_compatible_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}kernel_side_typed_consumer_compatible_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_single_field_handle_handoff_canary_current_wna16_arg_compatible_count": _as_int(
            aggregate.get(
                f"{SINGLE_FIELD_HANDLE_HANDOFF_CANARY_PREFIX}current_wna16_arg_compatible_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_checked_count": _as_int(
            aggregate.get(f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}checked_count")
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_ready_count": _as_int(
            aggregate.get(f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}ready_count")
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_hash_checked_count": _as_int(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}hash_checked_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_semantic_adapter_hash_checked_count": _as_int(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}semantic_adapter_hash_checked_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_mode": str(
            aggregate.get(f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}mode")
            or ""
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_row_count": _as_int(
            aggregate.get(f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}row_count")
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_column_count_max": _as_int(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}column_count_max"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_kernel_side_schema_name": str(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}kernel_side_schema_name"
            )
            or ""
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_kernel_side_schema_hash": str(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}kernel_side_schema_hash"
            )
            or ""
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_kernel_side_field_count": _as_int(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}kernel_side_field_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_consumer_schema_present_count": _as_int(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}consumer_schema_present_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_consumer_connected_count": _as_int(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}consumer_connected_count"
            )
        ),
        ...
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_blocked_count": _as_int(
            aggregate.get(f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}blocked_count")
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_block_reason": str(
            aggregate.get(f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}block_reason")
            or ""
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_payload_bytes": _as_int(
            aggregate.get(f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}payload_bytes")
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_passed_to_kernel_count": _as_int(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}passed_to_kernel_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_kernel_arg_violation_count": _as_int(
            aggregate.get(
                f"{KERNEL_SIDE_CONSUMER_SCHEMA_ADAPTER_PREFIX}kernel_arg_violation_count"
            )
        ),
        "premap_consumer_descriptor_prep_consumer_shim_kernel_side_consumer_schema_adapter_live_compatible_with_current_wna16_args_count": _as_int(
            aggregate.get(
    ...
    return {
        ...
        "allow_single_field_replacement_live": bool(
            allow_single_field_replacement_live
        ),
        "allow_single_field_replacement_prepared_table_candidate_source": bool(
            allow_single_field_replacement_prepared_table_candidate_source
        ),
        "allow_single_field_replacement_wrapper_only_canary": bool(
            allow_single_field_replacement_wrapper_only_canary
        ),
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Require the live-disabled future kernel-side consumer schema "
            "adapter. This is stricter than the semantic handle adapter: the "
            "schema must be present and ready, while live handoff, payload, "
            "current WNA16 compatibility, and kernel argument mutation remain "
            "disabled."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--allow-single-field-replacement-wrapper-only-canary",
        action="store_true",
        help=(
            "Canary-only mode: validate only the WNA16 wrapper-level "
            "single-field live/fallback counters. This deliberately does not "
            "require the full premap consumer mapping/descriptor-prep event "
            "stream, and must not be used as a lab precondition gate."
        ),
    )
    parser.add_argument(
        "--expected-single-field-replacement-allow-signature-mismatch-live",
        choices=("true", "false"),
        help=(
            "Optional strict check for the runtime "
            "single_field_replacement_allow_signature_mismatch_live flag. Use "
            "this for canary gate artifacts that intentionally exercise the "
            "signature-mismatch branch while still requiring type-gated fallback."
        ),
    )
    ...
    result = check_summary(
        ...
        allow_single_field_replacement_wrapper_only_canary=(
            args.allow_single_field_replacement_wrapper_only_canary
        ),
        expected_single_field_replacement_allow_signature_mismatch_live=(
            None
            if args.expected_single_field_replacement_allow_signature_mismatch_live
            is None
            else args.expected_single_field_replacement_allow_signature_mismatch_live
            == "true"
        ),
    )
