...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--row-tile-stride", type=int, default=257)
...
def write_rocprof_input(path: Path, *, groups: list[list[str]], device: int, kernel_regex: str) -> None:
    lines = ["# Generated by scripts/run_rocwmma_rocprof_classification.py"]
    for group in groups:
        lines.append("pmc : " + " ".join(group))
    lines.extend([f"gpu: {device}", f"kernel: {kernel_regex}", ""])
...
def kernel_regex_for_mode(mode: str) -> str:
    return {
        "global_frag_reuse": "global_frag_reuse_kernel",
        "global_reload_per_row": "global_reload_per_row_kernel",
        "global_reload_distinct_per_row": "global_reload_distinct_per_row_kernel",
        "lds_hit": "lds_hit_kernel",
        "lds_miss_overwrite": "lds_miss_overwrite_kernel",
    }[mode]
...
def main() -> None:
    ...
    for device in devices:
        for mode in modes:
            input_file = args.output_dir / f"rocprof_input_gpu{device}_{mode}.txt"
            write_rocprof_input(input_file, groups=groups, device=device, kernel_regex=kernel_regex_for_mode(mode))
