#!/usr/bin/env python3
"""Build a payload-copy completion-blocked artifact from execution-blocked rows."""
...
def build_completion_blocked_artifact(
    *,
    copy_descriptor_execution_blocked_json: str | Path,
    output_json: str | Path,
    completion_capacity: int,
    require_pass: bool = False,
) -> dict[str, Any]:
    source_path = _resolve(copy_descriptor_execution_blocked_json)
    ...
    execution_queue_planned_payload_bytes = _int_metric(
        source,
        "execution_queue_planned_payload_bytes",
    )
    ...
    if (
        ...
        or execution_queue_planned_payload_bytes is None
        or planned_payload_bytes != execution_queue_planned_payload_bytes
    ...
    completion_capacity_value = int(completion_capacity)
    if completion_capacity_value <= 0:
        failures.append("completion_capacity_invalid")
    if descriptor_count is not None and completion_capacity_value < descriptor_count:
        failures.append("completion_capacity_too_small")

    completion_queue_row_count = int(descriptor_count or 0)
    completion_queue_planned_payload_bytes = int(planned_payload_bytes or 0)
    passed = not failures
    payload = {
        ...
        "passed": passed,
        "failures": failures,
        "copy_completion_blocked_ready": passed,
        "source_copy_descriptor_execution_blocked_json": str(source_path),
        "source_copy_descriptor_execution_blocked_sha256": _sha256(source_path),
        ...
        "copy_descriptor_count": completion_queue_row_count,
        ...
        "planned_payload_bytes": completion_queue_planned_payload_bytes,
        ...
        "completion_queue_shape_checked": passed,
        "completion_capacity": completion_capacity_value,
        "completion_queue_row_count": completion_queue_row_count,
        "completion_queue_planned_payload_bytes": completion_queue_planned_payload_bytes,
        ...
        "live_runtime_instantiated": False,
        "requires_payload_runtime": False,
        "execution_mode": "payload_cache_copy_completion_blocked",
        "next_runtime_stage": "payload_cache_ready_credit_blocked",
    }
    ...
    if require_pass and not passed:
        raise SystemExit(f"completion-blocked artifact failed validation: {failures}")
    return payload


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--copy-descriptor-execution-blocked-json", required=True)
    parser.add_argument("--output-json", required=True)
    parser.add_argument("--completion-capacity", type=int, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = build_completion_blocked_artifact(
        copy_descriptor_execution_blocked_json=args.copy_descriptor_execution_blocked_json,
        output_json=args.output_json,
        completion_capacity=args.completion_capacity,
