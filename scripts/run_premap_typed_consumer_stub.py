...
def validate_macros(macros: list[str]) -> list[str]:
    ...
    if (
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_WNA16_ADJACENT_TYPED_SLOT_ABI"
        in normalized
        and "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ENDPOINT_PTR_ABI"
        not in normalized
    ):
        raise ValueError(
            "future native consumer WNA16-adjacent typed slot ABI requires "
            "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_ENDPOINT_PTR_ABI"
        )
    if (
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_TYPED_SLOT_KERNEL_VARIANT_ABI"
        in normalized
        and "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_WNA16_ADJACENT_TYPED_SLOT_ABI"
        not in normalized
    ):
        raise ValueError(
            "future WNA16 typed-slot kernel variant ABI requires "
            "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_WNA16_ADJACENT_TYPED_SLOT_ABI"
        )
    if (
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_KERNEL_ACCEPT_TYPED_SLOT_ABI"
        ...
        raise ValueError(
            "future WNA16 kernel accept-typed-slot ABI requires "
            "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_WNA16_ADJACENT_TYPED_SLOT_ABI"
        )
    if (
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_KERNEL_SIDE_CONSUMER_EXECUTION_ABI"
        in normalized
        and "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_KERNEL_ACCEPT_TYPED_SLOT_ABI"
        not in normalized
    ):
        raise ValueError(
            "future WNA16 kernel-side consumer execution ABI requires "
            "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_KERNEL_ACCEPT_TYPED_SLOT_ABI"
        )
    if (
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_SINGLE_FIELD_HANDOFF_CANARY_ABI"
        in normalized
        and "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_KERNEL_SIDE_CONSUMER_EXECUTION_ABI"
        not in normalized
    ):
        raise ValueError(
            "future WNA16 single-field handoff canary ABI requires "
            "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_KERNEL_SIDE_CONSUMER_EXECUTION_ABI"
        )
    if (
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_WNA16_SIDE_CONSUMER_VARIANT_EXECUTION_ABI"
        in normalized
        and "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_TYPED_SLOT_KERNEL_VARIANT_ABI"
        not in normalized
    ):
        raise ValueError(
            "WNA16-side consumer variant execution ABI requires "
            "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_WNA16_TYPED_SLOT_KERNEL_VARIANT_ABI"
        )
    if (
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_REQUEST_PTR_ABI"
    ...
    if (
        "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_REQUEST_LAUNCH_PTR_ABI"
        in normalized
        and "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_REQUEST_LAUNCH_ABI"
        not in normalized
    ):
        raise ValueError(
            "future native consumer request launch pointer ABI requires "
            "MTP_PREMAP_TYPED_CONSUMER_CHECK_FUTURE_KERNEL_NATIVE_CONSUMER_REQUEST_LAUNCH_ABI"
        )
    return normalized
