#!/usr/bin/env python3
"""Build a native execution adapter blocked artifact from ready-credit rows."""
...
def build_native_execution_adapter_blocked_artifact(
    *,
    ready_credit_blocked_json: str | Path,
    output_json: str | Path,
    adapter_capacity: int,
    require_pass: bool = False,
) -> dict[str, Any]:
    source_path = _resolve(ready_credit_blocked_json)
    ...
    ready_credit_queue_row_count = _int_metric(source, "ready_credit_queue_row_count")
    ...
    ready_credit_capacity = _int_metric(source, "ready_credit_capacity")
    ...
    ready_credit_queue_planned_payload_bytes = _int_metric(
        source,
        "ready_credit_queue_planned_payload_bytes",
    )
    ...
    for key, value in (
        ...
        ("ready_credit_capacity", ready_credit_capacity),
    ):
        if value is None or descriptor_count is None or value < descriptor_count:
    ...
    if (
        ...
        or ready_credit_queue_planned_payload_bytes is None
        or planned_payload_bytes != ready_credit_queue_planned_payload_bytes
    ...
    if adapter_capacity_value <= 0:
        failures.append("native_execution_adapter_capacity_invalid")
    if descriptor_count is not None and adapter_capacity_value < descriptor_count:
        failures.append("native_execution_adapter_capacity_too_small")

    adapter_row_count = int(descriptor_count or 0)
    adapter_planned_payload_bytes = int(planned_payload_bytes or 0)
    passed = not failures
    payload = {
        ...
        "passed": passed,
        "failures": failures,
        "native_execution_adapter_blocked_ready": passed,
        "source_ready_credit_blocked_json": str(source_path),
        "source_ready_credit_blocked_sha256": _sha256(source_path),
        ...
        "copy_descriptor_count": adapter_row_count,
        ...
        "planned_payload_bytes": adapter_planned_payload_bytes,
        ...
        "ready_credit_queue_row_count": int(ready_credit_queue_row_count or 0),
        "ready_credit_capacity": int(ready_credit_capacity or 0),
        "ready_credit_queue_planned_payload_bytes": int(
            ready_credit_queue_planned_payload_bytes or 0
        ),
        "native_execution_adapter_row_window_checked": passed,
        "native_execution_adapter_capacity": adapter_capacity_value,
        "native_execution_adapter_row_count": adapter_row_count,
        "native_execution_adapter_planned_payload_bytes": adapter_planned_payload_bytes,
        ...
        "live_runtime_instantiated": False,
        "requires_payload_runtime": False,
        "execution_mode": "payload_cache_native_execution_adapter_blocked",
        "next_runtime_stage": "real_payload_copy_canary_or_native_execution_payloadless_consumer",
    }
    ...
    if require_pass and not passed:
        raise SystemExit(
            f"native-execution-adapter-blocked artifact failed validation: {failures}"
        )
    return payload
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--ready-credit-blocked-json", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = build_native_execution_adapter_blocked_artifact(
        ready_credit_blocked_json=args.ready_credit_blocked_json,
