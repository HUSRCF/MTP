...
DEFAULT_CANARY_OUTPUT_DIR = (
    ...
    / "reports"
    / "premap_kernel_consumer"
    / "future_wna16_typed_slot_kernel_variant_second_field_handoff_canary_entry_args_ptr_default"
)
...
ENTRY_ARGS_PTR_MIRROR_FIELDS = HANDLE_FIELDS
KERNEL_SIDE_TYPED_PATH_PREFIX = "payloadless_future_wna16_kernel_side_typed_consumer_path"
FIELD_KINDS = {
    "descriptor_ptr": 1,
...
def _check_payloadless_kernel_side_evidence(first: dict[str, Any]) -> list[str]:
    failures: list[str] = []
    ...
    for key, expected in expected_scalars.items():
        if first.get(key) != expected:
            failures.append(f"first_{key}_mismatch:{first.get(key)!r}!={expected!r}")
    kernel_source_count = _int_metric(first, f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_source_count")
    if kernel_source_count is None:
        failures.append("first_payloadless_kernel_side_source_count_invalid")
    elif source_count is not None and kernel_source_count != source_count:
        failures.append("first_payloadless_kernel_side_source_count_mismatch")
    kernel_input_count = _int_metric(first, f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_input_json_count")
    if kernel_input_count is None:
        failures.append("first_payloadless_kernel_side_input_count_invalid")
    elif source_count is not None and kernel_input_count != source_count:
        failures.append("first_payloadless_kernel_side_input_count_mismatch")
    kernel_row_count = _int_metric(first, f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_row_count")
    kernel_row_ok_count = _int_metric(first, f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_row_ok_count")
    if kernel_row_count is None:
        failures.append("first_payloadless_kernel_side_row_count_invalid")
    elif row_count is not None and kernel_row_count != row_count:
        failures.append("first_payloadless_kernel_side_row_count_mismatch")
    if kernel_row_ok_count is None:
        failures.append("first_payloadless_kernel_side_row_ok_count_invalid")
    elif kernel_row_count is not None and kernel_row_ok_count != kernel_row_count:
        failures.append("first_payloadless_kernel_side_row_ok_count_mismatch")
    path_key = f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_evidence_path"
    sha_key = f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_evidence_sha256"
    evidence_path_value = first.get(path_key)
    evidence_sha_value = first.get(sha_key)
    evidence_payload: dict[str, Any] | None = None
    if not isinstance(evidence_path_value, str) or not evidence_path_value:
        failures.append("first_payloadless_kernel_side_evidence_path_missing")
    elif not _is_sha256_hex(evidence_sha_value):
        failures.append("first_payloadless_kernel_side_evidence_sha_invalid")
    else:
        ...
        try:
            ...
            failures.append(
                "first_payloadless_kernel_side_evidence_sha_failed:"
                f"{exc.__class__.__name__}:{exc}"
            )
        else:
            if actual_sha != evidence_sha_value:
                failures.append("first_payloadless_kernel_side_evidence_sha_mismatch")
            else:
                try:
                    evidence_payload = _load_json(evidence_path)
                except Exception as exc:
                    failures.append(
                        "first_payloadless_kernel_side_evidence_json_invalid:"
                        f"{exc.__class__.__name__}:{exc}"
                    )
    ...
    for key, expected in expected_evidence.items():
        if evidence_payload.get(key) != expected:
            failures.append(
                f"first_payloadless_kernel_side_evidence_{key}_mismatch:"
                f"{evidence_payload.get(key)!r}!={expected!r}"
            )
    if evidence_payload.get("failures") != []:
        failures.append("first_payloadless_kernel_side_evidence_failures_not_empty")
    if source_count is not None and _int_metric(evidence_payload, "source_count") != source_count:
        failures.append("first_payloadless_kernel_side_evidence_source_count_mismatch")
    if kernel_input_count is not None and _int_metric(evidence_payload, "input_json_count") != kernel_input_count:
        failures.append("first_payloadless_kernel_side_evidence_input_count_mismatch")
    if row_count is not None and _int_metric(evidence_payload, "row_count") != row_count:
        failures.append("first_payloadless_kernel_side_evidence_row_count_mismatch")
    if row_count is not None and _int_metric(evidence_payload, "row_ok_count") != row_count:
        failures.append("first_payloadless_kernel_side_evidence_row_ok_count_mismatch")
    all_four_sha = first.get(f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_all_four_sha256")
    if not _is_sha256_hex(all_four_sha):
        failures.append("first_payloadless_kernel_side_all_four_sha_invalid")
    elif evidence_payload.get("all_four_sha256") != all_four_sha:
        failures.append("first_payloadless_kernel_side_evidence_all_four_sha_mismatch")
    selected_manifest = first.get(
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_selected_input_manifest_sha256"
    )
    if not _is_sha256_hex(selected_manifest):
        failures.append("first_payloadless_kernel_side_selected_manifest_invalid")
    elif evidence_payload.get("selected_input_manifest_sha256") != selected_manifest:
        failures.append("first_payloadless_kernel_side_evidence_selected_manifest_mismatch")
    return failures


def _check_entry_args_ptr_payloadless_artifact(
    ...
    *,
    ...
    for key, expected in expected_scalars.items():
        if payloadless.get(key) != expected:
            failures.append(
                f"{prefix}_{key}_mismatch:{payloadless.get(key)!r}!={expected!r}"
            )
    if list(payloadless.get("entry_args_ptr_sweep_mirror_fields") or []) != list(
        ENTRY_ARGS_PTR_MIRROR_FIELDS
    ):
        failures.append(f"{prefix}_entry_args_ptr_sweep_mirror_fields_mismatch")
    sweep_rows = _int_metric(payloadless, "entry_args_ptr_sweep_row_count")
    check_rows = _int_metric(payloadless, "entry_args_ptr_sweep_check_row_count")
    if sweep_rows is None or sweep_rows < min_row_count:
        failures.append(f"{prefix}_entry_args_ptr_sweep_row_count_invalid")
    if check_rows is None or check_rows < min_row_count:
        failures.append(f"{prefix}_entry_args_ptr_sweep_check_row_count_invalid")
    if sweep_rows is not None and check_rows is not None and sweep_rows != check_rows:
        failures.append(f"{prefix}_entry_args_ptr_sweep_check_row_count_mismatch")
    if row_count is not None and row_count < min_row_count:
        failures.append(f"{prefix}_row_count_below_entry_args_ptr_min")
    if source_count is not None and source_count <= 0:
        failures.append(f"{prefix}_source_count_invalid_for_entry_args_ptr")
    loaded_artifacts: dict[str, tuple[Path, dict[str, Any]]] = {}
    for path_key, sha_key in (
        ("entry_args_ptr_sweep_json", "entry_args_ptr_sweep_sha256"),
        ("entry_args_ptr_sweep_check_json", "entry_args_ptr_sweep_check_sha256"),
    ):
        ...
        try:
            ...
            if actual_sha != sha_value:
                ...
                try:
                    loaded_artifacts[path_key] = (path, _load_json(path))
                except Exception as exc:
                    failures.append(
                        f"{prefix}_{path_key}_load_failed:"
                        f"{exc.__class__.__name__}:{exc}"
                    )
    sweep_entry = loaded_artifacts.get("entry_args_ptr_sweep_json")
    check_entry = loaded_artifacts.get("entry_args_ptr_sweep_check_json")
    if sweep_entry is not None:
        _sweep_path, sweep = sweep_entry
        expected_sweep = {
            ...
            "mirror_fields": list(ENTRY_ARGS_PTR_MIRROR_FIELDS),
        }
        for key, expected in expected_sweep.items():
            if sweep.get(key) != expected:
                failures.append(
                    f"{prefix}_entry_args_ptr_sweep_{key}_mismatch:"
                    f"{sweep.get(key)!r}!={expected!r}"
                )
        row_counts = sweep.get("row_counts")
        if not isinstance(row_counts, dict):
            failures.append(f"{prefix}_entry_args_ptr_sweep_row_counts_missing")
        else:
            values: list[int] = []
            for field in ENTRY_ARGS_PTR_MIRROR_FIELDS:
                value = row_counts.get(field)
                if not isinstance(value, int) or isinstance(value, bool):
                    failures.append(
                        f"{prefix}_entry_args_ptr_sweep_{field}_row_count_invalid"
                    )
                    continue
                values.append(value)
            if values:
                if any(value != values[0] for value in values):
                    failures.append(
                        f"{prefix}_entry_args_ptr_sweep_field_row_counts_not_equal"
                    )
                if sweep_rows is not None and values[0] != sweep_rows:
                    failures.append(
                        f"{prefix}_entry_args_ptr_sweep_row_count_payloadless_mismatch"
                    )
                if values[0] <= 512:
                    failures.append(
                        f"{prefix}_entry_args_ptr_sweep_row_count_not_larger_than_window"
                    )
        field_reports = sweep.get("field_reports")
        if not isinstance(field_reports, dict):
            failures.append(f"{prefix}_entry_args_ptr_sweep_field_reports_missing")
        else:
            for field in ENTRY_ARGS_PTR_MIRROR_FIELDS:
                report = field_reports.get(field)
                if not isinstance(report, dict):
                    failures.append(
                        f"{prefix}_entry_args_ptr_sweep_{field}_field_report_missing"
                    )
                    continue
                expected_report = {
                    "passed": True,
                ...
                for key, expected in expected_report.items():
                    if report.get(key) != expected:
                        failures.append(
                            f"{prefix}_entry_args_ptr_sweep_{field}_{key}_mismatch:"
                            f"{report.get(key)!r}!={expected!r}"
                        )
                for key in ("sweep_json", "check_json"):
                    if not isinstance(report.get(key), str) or not report.get(key):
                        failures.append(
                            f"{prefix}_entry_args_ptr_sweep_{field}_{key}_missing"
                        )
    if check_entry is not None:
        check_path, check = check_entry
        expected_check = {
            ...
            "mirror_fields_checked": list(ENTRY_ARGS_PTR_MIRROR_FIELDS),
        }
        for key, expected in expected_check.items():
            if check.get(key) != expected:
                failures.append(
                    f"{prefix}_entry_args_ptr_check_{key}_mismatch:"
                    f"{check.get(key)!r}!={expected!r}"
                )
        if check.get("row_count") != sweep_rows:
            failures.append(f"{prefix}_entry_args_ptr_check_row_count_sweep_mismatch")
        sweep_path_value = payloadless.get("entry_args_ptr_sweep_json")
        check_sweep_path = check.get("all_field_window_sweep_json")
        if not isinstance(check_sweep_path, str) or not check_sweep_path:
            failures.append(f"{prefix}_entry_args_ptr_check_sweep_path_missing")
        elif isinstance(sweep_path_value, str):
            expected_path = _resolve(sweep_path_value).resolve()
            actual_path = _resolve(check_sweep_path).resolve()
            if actual_path != expected_path:
                failures.append(
                    f"{prefix}_entry_args_ptr_check_sweep_path_mismatch:"
                    f"{actual_path}!={expected_path}"
                )
        if not check_path.is_file():
            failures.append(f"{prefix}_entry_args_ptr_check_path_not_file")
    return failures
...
def _check_first_field(
    ...
    *,
    ...
    if payloadless is not None:
        ...
        failures.extend(
            _check_entry_args_ptr_payloadless_artifact(
                payloadless,
                source_count=source_count,
                row_count=row_count,
                min_row_count=min_row_count,
                prefix="first_payloadless_execution",
            )
        )
    all_four_expected = {
    ...
    failures.extend(_check_payloadless_kernel_side_evidence(first))
...
def _payloadless_artifact_for_summary(first: dict[str, Any]) -> dict[str, Any]:
    path_value = first.get("payloadless_execution_json")
    sha_value = first.get("payloadless_execution_sha256")
    if not isinstance(path_value, str) or not path_value or not _is_sha256_hex(sha_value):
        return {}
    path = _resolve(path_value)
    try:
...
def run_second_field_handoff_canary(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payloadless_summary = _payloadless_artifact_for_summary(first)
    report: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": (
        ...
        "payloadless_execution_native_artifact_ready": payloadless_summary.get(
            "payloadless_execution_native_artifact_ready"
        ),
        "payloadless_execution_lab_preflight_ready": payloadless_summary.get(
            "payloadless_execution_lab_preflight_ready"
        ),
        "payloadless_entry_args_ptr_required": payloadless_summary.get(
            "entry_args_ptr_required"
        ),
        "payloadless_entry_args_ptr_sweep_json": payloadless_summary.get(
            "entry_args_ptr_sweep_json"
        ),
        "payloadless_entry_args_ptr_sweep_sha256": payloadless_summary.get(
            "entry_args_ptr_sweep_sha256"
        ),
        "payloadless_entry_args_ptr_sweep_check_json": payloadless_summary.get(
            "entry_args_ptr_sweep_check_json"
        ),
        "payloadless_entry_args_ptr_sweep_check_sha256": payloadless_summary.get(
            "entry_args_ptr_sweep_check_sha256"
        ),
        "payloadless_entry_args_ptr_sweep_row_count": payloadless_summary.get(
            "entry_args_ptr_sweep_row_count"
        ),
        "payloadless_entry_args_ptr_sweep_check_row_count": payloadless_summary.get(
            "entry_args_ptr_sweep_check_row_count"
        ),
        "payloadless_entry_args_ptr_sweep_mirror_fields": payloadless_summary.get(
        ...
        "payloadless_future_wna16_kernel_side_typed_consumer_path_ready": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_ready"
        ),
        "payloadless_future_wna16_kernel_side_typed_consumer_path_hashes_valid": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_hashes_valid"
        ),
        "payloadless_future_wna16_kernel_side_typed_consumer_path_evidence_path": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_evidence_path"
        ),
        "payloadless_future_wna16_kernel_side_typed_consumer_path_evidence_sha256": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_evidence_sha256"
        ),
        "payloadless_future_wna16_kernel_side_typed_consumer_path_source_count": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_source_count"
        ),
        "payloadless_future_wna16_kernel_side_typed_consumer_path_input_json_count": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_input_json_count"
        ),
        "payloadless_future_wna16_kernel_side_typed_consumer_path_row_count": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_row_count"
        ),
        "payloadless_future_wna16_kernel_side_typed_consumer_path_row_ok_count": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_row_ok_count"
        ),
        "payloadless_future_wna16_kernel_side_typed_consumer_path_all_four_sha256": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_all_four_sha256"
        ),
        "payloadless_future_wna16_kernel_side_typed_consumer_path_selected_input_manifest_sha256": first.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_selected_input_manifest_sha256"
        ),
