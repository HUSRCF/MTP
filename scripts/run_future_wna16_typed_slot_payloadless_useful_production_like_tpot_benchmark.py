#!/usr/bin/env python3
"""Run or archive a production-like vLLM TPOT baseline behind the timing gate.

This benchmark wrapper is intentionally conservative.  It consumes the
production-like timing readiness gate, then either reuses an existing
``performance_summary.json`` or launches the validated vLLM config into a fresh
output directory.  The result is a production-like baseline artifact only; it
does not enable the payloadless useful typed-slot path and does not claim a
future WNA16 typed-slot speedup.
"""
...
BENCHMARK_NAME = "premap_future_wna16_typed_slot_payloadless_useful_production_like_tpot_baseline_v1"
BENCHMARK_MODE = "production_like_baseline_only"
BENCHMARK_SOURCE = "premap_future_wna16_typed_slot_payloadless_useful_production_like_timing_gate_v1"
...
EXPECTED_GATE_FLAGS: dict[str, Any] = {
    ...
    "timing_gate_name": "premap_future_wna16_typed_slot_payloadless_useful_production_like_timing_gate_v1",
    "timing_gate_mode": "production_like_config_readiness_gate",
    "timing_gate_source": "premap_future_wna16_typed_slot_payloadless_useful_runtime_ablation_v1",
    "passed": True,
    "failures": [],
...
def _check_gate(gate: dict[str, Any], failures: list[str]) -> None:
    ...
    if not isinstance(trace_config, str) or not trace_config:
        failures.append("timing_gate_trace_config_missing")
        return
    trace_config_path = _resolve(trace_config)
    if not trace_config_path.exists():
        failures.append("timing_gate_trace_config_missing")
        return
    if _sha256(trace_config_path) != trace_sha:
        failures.append("timing_gate_trace_config_sha256_mismatch")


def _default_trace_dir_from_gate(gate: dict[str, Any]) -> Path | None:
    summary = gate.get("trace_config_summary")
...
def _copy_config_for_run(
    *,
    ...
    trace_dir = output_root / "production_like_baseline" / f"repeat_{repeat:02d}"
    ...
    config_out = output_root / "trace_configs" / f"production_like_baseline_repeat_{repeat:02d}.yaml"
...
def _run_trace(
    *,
    ...
    command = [python, "scripts/trace_router_mtp_vllm.py", str(config_path)]
    proc = subprocess.run(
        command,
        cwd=REPO_ROOT,
        env=env,
...
def run_production_like_tpot_benchmark(args: argparse.Namespace) -> dict[str, Any]:
    gate_path = _resolve(args.timing_gate_json)
    ...
    try:
        ...
        gate = {}
        failures.append(f"timing_gate_load_failed:{exc.__class__.__name__}:{exc}")
    if gate:
    ...
    if failures:
        ...
        log_path = output_root / "logs" / f"production_like_baseline_repeat_{args.repeat:02d}.log"
        run_executed = True
        ...
        if trace_dir is None:
            ...
            failures.append("timing_gate_trace_dir_missing")
        elif not _same_path(trace_dir, gate_trace_dir):
            failures.append("trace_dir_not_bound_to_timing_gate")
    ...
    result: dict[str, Any] = {
        ...
        "passed": passed,
        "failures": failures,
        "production_like_tpot_baseline_ready": passed,
        "production_like_benchmark_config_ready": bool(
            gate.get("production_like_benchmark_config_ready")
        ),
        "timing_gate_json": str(gate_path),
        "timing_gate_sha256": _sha256(gate_path) if gate_path.exists() else None,
        "trace_config": gate.get("trace_config"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--timing-gate-json", default=str(DEFAULT_TIMING_GATE_JSON))
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_production_like_tpot_benchmark(args)
