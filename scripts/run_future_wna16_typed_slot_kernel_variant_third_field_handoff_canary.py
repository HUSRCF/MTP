#!/usr/bin/env python3
"""Run the future WNA16 typed-slot third-field handoff canary gate.

This stage requires the second-field canary gate to have passed on
``aux_metadata_handle``. It reuses that gate's persisted native runner
``input_jsons`` and runs the independent future typed-slot native path on the
third field, defaulting to ``packed_weight_descriptor``. It remains readonly:
...
CANARY_MODE = "readonly_future_wna16_typed_slot_third_field_handoff_canary"
CANARY_SOURCE = "premap_future_wna16_typed_slot_second_field_handoff_canary_v1"
PREVIOUS_FIELD = "aux_metadata_handle"
FIRST_FIELD = "scale_metadata_handle"
DEFAULT_THIRD_FIELD = "packed_weight_descriptor"
KERNEL_SIDE_TYPED_PATH_PREFIX = (
    "payloadless_future_wna16_kernel_side_typed_consumer_path"
)
NEXT_RUNTIME_STAGE = (
...
def _raw_runner_expected(field: str) -> dict[str, Any]:
    ...
    return {
        "passed": True,
        "source": previous_gate.RUNNER_SOURCE,
        "mirror_field": field,
        ...
        "future_wna16_single_field_handoff_canary_name": previous_gate.SINGLE_FIELD_NAME,
        "future_wna16_single_field_handoff_canary_abi_name": (
            previous_gate.SINGLE_FIELD_NAME
        ),
        "future_wna16_single_field_handoff_canary_mode": previous_gate.SINGLE_FIELD_MODE,
        "future_wna16_single_field_handoff_canary_source": (
            previous_gate.SINGLE_FIELD_SOURCE
        ),
        "future_wna16_single_field_handoff_canary_field_read_path": (
            previous_gate.SINGLE_FIELD_READ_PATH
        ),
...
def _check_payloadless_kernel_side_evidence(
    previous: dict[str, Any],
    *,
    previous_path: Path,
    source_count: int | None,
    ...
    failures: list[str] = []
    prefix = KERNEL_SIDE_TYPED_PATH_PREFIX
    expected_scalars = {
        f"{prefix}_ready": True,
        f"{prefix}_hashes_valid": True,
    }
    for key, expected in expected_scalars.items():
        if previous.get(key) != expected:
            failures.append(
                f"previous_{key}_mismatch:{previous.get(key)!r}!={expected!r}"
            )
    kernel_source_count = _int_metric(previous, f"{prefix}_source_count")
    if kernel_source_count is None:
        failures.append("previous_payloadless_kernel_side_source_count_invalid")
    elif source_count is not None and kernel_source_count != source_count:
        failures.append("previous_payloadless_kernel_side_source_count_mismatch")
    kernel_input_count = _int_metric(previous, f"{prefix}_input_json_count")
    if kernel_input_count is None:
        failures.append("previous_payloadless_kernel_side_input_count_invalid")
    elif source_count is not None and kernel_input_count != source_count:
        failures.append("previous_payloadless_kernel_side_input_count_mismatch")
    kernel_row_count = _int_metric(previous, f"{prefix}_row_count")
    kernel_row_ok_count = _int_metric(previous, f"{prefix}_row_ok_count")
    if kernel_row_count is None:
        failures.append("previous_payloadless_kernel_side_row_count_invalid")
    elif row_count is not None and kernel_row_count != row_count:
        failures.append("previous_payloadless_kernel_side_row_count_mismatch")
    if kernel_row_ok_count is None:
        failures.append("previous_payloadless_kernel_side_row_ok_count_invalid")
    elif kernel_row_count is not None and kernel_row_ok_count != kernel_row_count:
        failures.append("previous_payloadless_kernel_side_row_ok_count_mismatch")

    evidence_path_value = previous.get(f"{prefix}_evidence_path")
    evidence_sha_value = previous.get(f"{prefix}_evidence_sha256")
    evidence_path, path_failures = _resolve_artifact_reference(
        evidence_path_value,
        roots=[REPO_ROOT, previous_path.parent],
        label="previous_payloadless_kernel_side_evidence_path",
    )
    if path_failures:
        failures.extend(path_failures)
        return failures
    if not _is_sha256_hex(evidence_sha_value):
        failures.append("previous_payloadless_kernel_side_evidence_sha_invalid")
        return failures
    ...
    try:
        ...
        failures.append(
            "previous_payloadless_kernel_side_evidence_sha_failed:"
            f"{exc.__class__.__name__}:{exc}"
        )
    else:
        if actual_sha != evidence_sha_value:
            failures.append("previous_payloadless_kernel_side_evidence_sha_mismatch")
        else:
            try:
                evidence_payload = _load_json(evidence_path)
            except Exception as exc:
                failures.append(
                    "previous_payloadless_kernel_side_evidence_json_invalid:"
                    f"{exc.__class__.__name__}:{exc}"
                )
    ...
    for key, expected in expected_evidence.items():
        if evidence_payload.get(key) != expected:
            failures.append(
                f"previous_payloadless_kernel_side_evidence_{key}_mismatch:"
                f"{evidence_payload.get(key)!r}!={expected!r}"
            )
    if evidence_payload.get("failures") != []:
        failures.append("previous_payloadless_kernel_side_evidence_failures_not_empty")
    if source_count is not None and _int_metric(evidence_payload, "source_count") != source_count:
        failures.append("previous_payloadless_kernel_side_evidence_source_count_mismatch")
    if kernel_input_count is not None and _int_metric(evidence_payload, "input_json_count") != kernel_input_count:
        failures.append("previous_payloadless_kernel_side_evidence_input_count_mismatch")
    if row_count is not None and _int_metric(evidence_payload, "row_count") != row_count:
        failures.append("previous_payloadless_kernel_side_evidence_row_count_mismatch")
    if row_count is not None and _int_metric(evidence_payload, "row_ok_count") != row_count:
        failures.append("previous_payloadless_kernel_side_evidence_row_ok_count_mismatch")
    all_four_sha = previous.get(f"{prefix}_all_four_sha256")
    if not _is_sha256_hex(all_four_sha):
        failures.append("previous_payloadless_kernel_side_all_four_sha_invalid")
    elif evidence_payload.get("all_four_sha256") != all_four_sha:
        failures.append("previous_payloadless_kernel_side_evidence_all_four_sha_mismatch")
    selected_manifest = previous.get(f"{prefix}_selected_input_manifest_sha256")
    if not _is_sha256_hex(selected_manifest):
        failures.append("previous_payloadless_kernel_side_selected_manifest_invalid")
    elif evidence_payload.get("selected_input_manifest_sha256") != selected_manifest:
        failures.append(
            "previous_payloadless_kernel_side_evidence_selected_manifest_mismatch"
        )
    return failures
...
def _check_previous_gate(
    ...
    *,
    ...
    expected = {
        ...
        "second_field_name": PREVIOUS_FIELD,
    ...
    if row_count is not None and previous.get("second_field_handoff_canary_runner_row_count") != row_count:
        failures.append("previous_runner_row_count_mismatch")
    if row_count is not None and previous.get("second_field_handoff_canary_runner_row_ok_count") != row_count:
        failures.append("previous_runner_row_ok_count_mismatch")
    if not _is_hex_u64(previous.get("second_field_handoff_field_read_hash")):
        failures.append("previous_second_field_read_hash_invalid")
    if not _is_hex_u64(previous.get("second_field_handoff_canary_runner_hash")):
    ...
    if isinstance(payloadless_hashes, dict):
        if payloadless_hashes.get(PREVIOUS_FIELD) != previous.get(
            "second_field_handoff_field_read_hash"
        ):
            failures.append("previous_payloadless_second_field_hash_mismatch")
        if not _is_hex_u64(payloadless_hashes.get(third_field)):
            failures.append("previous_payloadless_default_third_field_hash_invalid")
    ...
    failures.extend(
        _check_payloadless_kernel_side_evidence(
            previous,
            previous_path=previous_path,
            source_count=source_count,
            row_count=row_count,
    ...
    underlying_path, path_failures = _resolve_artifact_reference(
        previous.get("second_field_underlying_json"),
        roots=[REPO_ROOT, previous_path.parent],
        label="previous_second_field_underlying_json",
    )
    if path_failures:
        failures.extend(path_failures)
    elif not _is_sha256_hex(underlying_sha_value):
        failures.append("previous_second_field_underlying_sha256_invalid")
    else:
        ...
        try:
            ...
            failures.append(
                f"previous_second_field_underlying_load_failed:"
                f"{exc.__class__.__name__}:{exc}"
            )
        else:
            if actual_sha != underlying_sha_value:
                failures.append("previous_second_field_underlying_sha256_mismatch")
            ...
            failures.extend(
                _check_raw_runner(
                    ...
                    expected_hash=previous.get("second_field_handoff_canary_runner_hash"),
...
def _build_native_args(
    ...
    *,
    ...
    argv = [
        ...
        "--merged-output-json",
        str(canary_dir / "third_field_merged_input.json"),
        "--stub-output-json",
        str(canary_dir / "third_field_typed_consumer_stub.json"),
        "--output-json",
        str(canary_dir / "third_field_native_canary_runner.json"),
    ]
...
def _check_third_report(
    ...
    *,
    ...
    if not isinstance(payloadless_hashes, dict):
        failures.append("third_payloadless_field_read_hashes_missing")
    elif not _is_hex_u64(payloadless_hashes.get(third_field)):
        failures.append("third_payloadless_selected_field_hash_invalid")
    if not isinstance(payloadless_counts, dict):
        failures.append("third_payloadless_field_read_row_ok_counts_missing")
    elif payloadless_counts.get(third_field) != previous.get("row_count"):
        failures.append("third_payloadless_selected_field_row_ok_count_mismatch")
    return failures


def _check_third_underlying_json(
    ...
    if not underlying_json.exists():
        return None, ["third_field_underlying_json_missing"]
    try:
        underlying_sha256 = _sha256(underlying_json)
        underlying = _load_json(underlying_json)
    except Exception as exc:
        return None, [
            f"third_field_underlying_json_load_failed:{exc.__class__.__name__}:{exc}"
        ]
    checked_keys = (
        "passed",
        "source",
    ...
    for key in checked_keys:
        if underlying.get(key) != third.get(key):
            failures.append(
...
def run_third_field_handoff_canary(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.third_field in {FIRST_FIELD, PREVIOUS_FIELD}:
        failures.append("third_field_must_differ_from_previous_fields")
    if args.third_field != DEFAULT_THIRD_FIELD:
        failures.append("third_field_must_be_packed_weight_descriptor")
    input_paths: list[Path] = []
    ...
    third_report: dict[str, Any] | None = None
    third_outer_wall_ms: float | None = None
    if args.run_native_canary and previous_gate_ready:
        try:
            third_report, third_outer_wall_ms = _run_third_field_native(
                args,
                input_paths=input_paths,
            )
        except Exception as exc:  # pragma: no cover - exercised via tests
            failures.append(
                f"third_field_handoff_canary_exception:"
                f"{exc.__class__.__name__}:{exc}"
            )
        if third_report is not None:
            failures.extend(
                _check_third_report(
                    third_report,
                    previous=previous,
        ...
        failures.append("third_field_canary_skipped_due_to_previous_gate_failure")
    if args.require_native_canary and third_report is None:
        failures.append("third_field_handoff_canary_required_but_not_executed")
    passed = not failures
    canary_dir = _resolve(args.canary_output_dir)
    underlying_json = canary_dir / "third_field_native_canary_runner.json"
    underlying_sha256, underlying_failures = _check_third_underlying_json(
        underlying_json,
        third_report,
    )
    ...
    third_field_payloadless_hash = (
        payloadless_hashes.get(args.third_field)
        if isinstance(payloadless_hashes, dict)
        else None
    )
    third_field_payloadless_row_ok = (
        payloadless_counts.get(args.third_field)
        if isinstance(payloadless_counts, dict)
        else None
    )
    third_source_count = (
        third_report.get("selected_source_count")
        if third_report
        else previous.get("source_count")
    )
    third_row_count = (
        third_report.get("merged_row_count") if third_report else previous.get("row_count")
    )
    third_row_ok_count = (
        third_report.get("future_wna16_single_field_handoff_canary_row_ok_count")
        if third_report
        else None
    )
    report: dict[str, Any] = {
        ...
        "third_field_handoff_canary_name": CANARY_NAME,
        "third_field_handoff_canary_mode": CANARY_MODE,
        "third_field_handoff_canary_source": CANARY_SOURCE,
        "passed": passed,
        "failures": failures,
        ...
        "first_field_name": FIRST_FIELD,
        "second_field_name": PREVIOUS_FIELD,
        "second_field_read_hash": previous.get("second_field_handoff_field_read_hash"),
        "second_field_native_hash": previous.get("second_field_handoff_canary_runner_hash"),
        "third_field_name": args.third_field,
        "third_field_kind": FIELD_KINDS.get(str(args.third_field)),
        "third_field_mask": (
            1 << (FIELD_KINDS[str(args.third_field)] - 1)
            if args.third_field in FIELD_KINDS
            else None
        ),
        "third_field_underlying_json": str(underlying_json),
        ...
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_ready": previous.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_ready"
        ),
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_hashes_valid": previous.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_hashes_valid"
        ),
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_evidence_path": previous.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_evidence_path"
        ),
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_evidence_sha256": previous.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_evidence_sha256"
        ),
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_source_count": previous.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_source_count"
        ),
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_input_json_count": previous.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_input_json_count"
        ),
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_row_count": previous.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_row_count"
        ),
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_row_ok_count": previous.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_row_ok_count"
        ),
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_all_four_sha256": previous.get(
            f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_all_four_sha256"
        ),
        f"{KERNEL_SIDE_TYPED_PATH_PREFIX}_selected_input_manifest_sha256": previous.get(
        ...
        "third_field_handoff_field_read_row_ok_count": third_field_payloadless_row_ok,
        "third_field_handoff_field_read_hash": third_field_payloadless_hash,
        "third_field_handoff_canary_runner_hash": (
            third_report.get("future_wna16_single_field_handoff_canary_hash_accumulator")
            if third_report
            else None
        ),
        "third_field_handoff_canary_runner_row_count": (
            third_report.get("future_wna16_single_field_handoff_canary_row_count")
            if third_report
            else None
        ),
        "third_field_handoff_canary_runner_row_ok_count": third_row_ok_count,
        "third_field_handoff_canary_native_requested": bool(args.run_native_canary),
        "third_field_handoff_canary_native_executed": third_report is not None,
        "third_field_handoff_canary_native_passed": (
            third_report.get("passed") if third_report else None
        ),
        "third_field_handoff_canary_outer_wall_ms": third_outer_wall_ms,
        "third_field_handoff_live_enabled": False,
        "third_field_handoff_block_reason": "third_field_handoff_live_disabled",
        "third_field_handoff_scope": (
            "independent_future_wna16_typed_slot_third_field_handoff_canary"
        ),
    ...
    if args.require_pass and not passed:
        raise SystemExit(
            "future WNA16 typed-slot third-field handoff canary failed: "
            + ", ".join(failures)
        )
    return report
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the future WNA16 typed-slot third-field handoff canary. "
            "This requires the aux-metadata second-field gate and keeps live "
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = run_third_field_handoff_canary(args)
