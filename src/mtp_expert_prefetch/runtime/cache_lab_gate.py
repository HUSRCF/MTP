...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadTransferToggleDisabledCanary:
    """Disabled payload-transfer toggle canary after payloadless adapter creation.

    This is the first contract object that exposes a future payload-transfer
    toggle.  The toggle is constructed but remains disabled; issuing a payload
    must be rejected and must not grant ready credit, pass kernel args, use the
    current WNA16 launch ABI, or measure vLLM latency.
    """
    ...
    manager_runtime_contract: str
    manager_runtime_mode: str
    payload_transfer_toggle_schema: str
    ...
    shifted_issue_unique_issue_key_count: int
    decision: str = "blocked"
    block_reason: str = "live_runtime_adapter_payload_transfer_toggle_disabled_canary_only"
    execution_mode: str = (
    ...
    def __post_init__(self) -> None:
        ...
        if self.manager_backend != "ReadyTimeExpertCacheManager":
            raise ValueError("payload-transfer toggle backend mismatch")
        if self.manager_runtime_contract != "ready_time_issue_demand_skeleton_v1":
            raise ValueError("payload-transfer toggle contract mismatch")
        if self.manager_runtime_mode != "ready_time_payload_cache_skeleton":
            raise ValueError("payload-transfer toggle mode mismatch")
        if (
            self.payload_transfer_toggle_schema
            != "ready_time_payload_cache_runtime_payload_transfer_toggle_disabled_canary_v1"
        ):
            raise ValueError("payload_transfer_toggle_schema mismatch")
        for field_name in (
            ...
            if getattr(self, field_name) is not True:
                raise ValueError(f"{field_name} must be true")
        if self.live_runtime_instantiated is not False:
            raise ValueError("live_runtime_instantiated must remain false")
        if self.decision != "blocked":
            raise ValueError("payload-transfer toggle canary decision must stay blocked")
        if (
            self.block_reason
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssuePlanDryRun:
    """Payload issue plan object built from a rejected source-bound request."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_request_blocked_canary: bool
    payload_issue_request_blocked_canary_status: str
    request_source: str
    request_layer_idx: int
    request_expert_idx: int
    requested_payload_bytes: int
    ...
    execution_mode: str = "payload_cache_live_runtime_adapter_payload_issue_plan_dry_run"
    live_payload_runtime_enabled: bool = False
    payload_transfer_runtime_enabled: bool = False
    payload_deref_allowed: bool = False
    payload_deref_runtime_allowed: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if self.stage != "payload_cache_live_runtime_adapter_payload_issue_plan_dry_run":
            raise ValueError("payload issue plan dry-run stage mismatch")
        if self.consumes_payload_issue_request_blocked_canary is not True:
            raise ValueError("payload issue plan must consume blocked request canary")
        if (
            not isinstance(self.payload_issue_request_blocked_canary_status, str)
            or not self.payload_issue_request_blocked_canary_status
        ):
            raise TypeError("payload_issue_request_blocked_canary_status must be nonempty")
        expected_status = (
            "blocked_by_payload_issue_request_blocked_canary:"
            f"{self.payload_issue_request_blocked_canary_status}"
        )
        if self.status != expected_status:
        ...
        for field_name in ("planned_issue_count", "issued_payload_count", "payload_bytes"):
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue plan dry-run decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue plan dry-run block reason mismatch")
        if self.execution_mode != "payload_cache_live_runtime_adapter_payload_issue_plan_dry_run":
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueQueueEntryDryRun:
    """Future queue-entry shape object that does not enqueue or issue payloads."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_executor_dry_run: bool
    payload_issue_executor_status: str
    payload_issue_queue_entry_schema: str
    payload_issue_queue_entry_created: bool
    payload_issue_executor_consumed: bool
    queue_entry_shape_checked: bool
    queue_entry_enqueued: bool
    queue_submit_allowed: bool
    request_source: str
    request_layer_idx: int
    request_expert_idx: int
    requested_payload_bytes: int
    ...
    decision: str = "blocked"
    block_reason: str = "payload_transfer_disabled"
    execution_mode: str = "payload_cache_live_runtime_adapter_payload_issue_queue_entry_dry_run"
    live_payload_runtime_enabled: bool = False
    payload_transfer_runtime_enabled: bool = False
    payload_deref_allowed: bool = False
    payload_deref_runtime_allowed: bool = False
    ...
    def __post_init__(self) -> None:
        if self.present is not True:
            raise ValueError("payload issue queue-entry dry-run must be present")
        if self.stage != "payload_cache_live_runtime_adapter_payload_issue_queue_entry_dry_run":
            raise ValueError("payload issue queue-entry dry-run stage mismatch")
        if self.consumes_payload_issue_executor_dry_run is not True:
            raise ValueError("payload issue queue-entry must consume executor dry-run")
        if (
            not isinstance(self.payload_issue_executor_status, str)
            or not self.payload_issue_executor_status
        ):
            raise TypeError("payload_issue_executor_status must be nonempty")
        if (
            self.payload_issue_executor_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_EXECUTOR_DRY_RUN_STATUS
        ):
            raise ValueError("payload issue queue-entry upstream ancestry status mismatch")
        expected_status = (
            "blocked_by_payload_issue_executor_dry_run:"
            f"{self.payload_issue_executor_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue queue-entry dry-run status mismatch")
        if (
            self.payload_issue_queue_entry_schema
            != "payload_cache_runtime_payload_issue_queue_entry_v1"
        ):
            raise ValueError("payload issue queue-entry schema mismatch")
        for field_name in (
        ...
        for field_name in ("queue_entry_enqueued", "queue_submit_allowed"):
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue queue-entry requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue queue-entry dry-run decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue queue-entry dry-run block reason mismatch")
        if (
            self.execution_mode
            != "payload_cache_live_runtime_adapter_payload_issue_queue_entry_dry_run"
        ):
            raise ValueError("payload issue queue-entry dry-run execution mode mismatch")
        for field_name in (
            "live_payload_runtime_enabled",
            "payload_transfer_runtime_enabled",
            "payload_deref_allowed",
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueQueueSubmitBlockedCanary:
    """Submit-gate canary that rejects a queue entry without enqueueing."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_queue_entry_dry_run: bool
    payload_issue_queue_entry_status: str
    payload_issue_queue_submit_schema: str
    payload_issue_queue_submit_canary_created: bool
    payload_issue_queue_entry_consumed: bool
    queue_submit_checked: bool
    queue_submit_rejected: bool
    queue_submit_allowed: bool
    queue_entry_enqueued: bool
    request_source: str
    request_layer_idx: int
    ...
    decision: str = "blocked"
    block_reason: str = "payload_transfer_disabled"
    execution_mode: str = "payload_cache_live_runtime_adapter_payload_issue_queue_submit_blocked_canary"
    live_payload_runtime_enabled: bool = False
    payload_transfer_runtime_enabled: bool = False
    payload_deref_allowed: bool = False
    payload_deref_runtime_allowed: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if self.consumes_payload_issue_queue_entry_dry_run is not True:
            raise ValueError("queue-submit canary must consume queue-entry dry-run")
        if (
            not isinstance(self.payload_issue_queue_entry_status, str)
            or not self.payload_issue_queue_entry_status
        ):
            raise TypeError("payload_issue_queue_entry_status must be nonempty")
        if (
            self.payload_issue_queue_entry_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_QUEUE_ENTRY_DRY_RUN_STATUS
        ):
            raise ValueError("payload issue queue-submit upstream ancestry status mismatch")
        expected_status = (
            "blocked_by_payload_issue_queue_entry_dry_run:"
            f"{self.payload_issue_queue_entry_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue queue-submit canary status mismatch")
        if (
            self.payload_issue_queue_submit_schema
        ...
        for field_name in ("queue_submit_allowed", "queue_entry_enqueued"):
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue queue-submit requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue queue-submit canary decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue queue-submit canary block reason mismatch")
        if (
            self.execution_mode
            != "payload_cache_live_runtime_adapter_payload_issue_queue_submit_blocked_canary"
        ):
            raise ValueError("payload issue queue-submit canary execution mode mismatch")
        for field_name in (
            "live_payload_runtime_enabled",
            "payload_transfer_runtime_enabled",
            "payload_deref_allowed",
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueSchedulerDispatchBlockedCanary:
    """Scheduler-dispatch canary that rejects in-flight payload work."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_inflight_admission_blocked_canary: bool
    payload_issue_inflight_admission_status: str
    payload_issue_scheduler_dispatch_schema: str
    payload_issue_scheduler_dispatch_canary_created: bool
    payload_issue_inflight_admission_consumed: bool
    scheduler_dispatch_checked: bool
    scheduler_dispatch_rejected: bool
    scheduler_dispatch_allowed: bool
    scheduler_dispatch_enqueued: bool
    request_source: str
    request_layer_idx: int
    request_expert_idx: int
    requested_payload_bytes: int
    ...
    def __post_init__(self) -> None:
        if self.present is not True:
            raise ValueError("payload issue scheduler-dispatch canary must be present")
        if (
            self.stage
            != "payload_cache_live_runtime_adapter_payload_issue_scheduler_dispatch_blocked_canary"
        ):
            raise ValueError("payload issue scheduler-dispatch canary stage mismatch")
        if self.consumes_payload_issue_inflight_admission_blocked_canary is not True:
            raise ValueError("scheduler dispatch must consume in-flight admission canary")
        if (
            not isinstance(self.payload_issue_inflight_admission_status, str)
            or not self.payload_issue_inflight_admission_status
        ):
            raise TypeError("payload_issue_inflight_admission_status must be nonempty")
        if (
            self.payload_issue_inflight_admission_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_INFLIGHT_ADMISSION_BLOCKED_CANARY_STATUS
        ):
            raise ValueError(
                "payload issue scheduler-dispatch upstream ancestry status mismatch",
            )
        expected_status = (
            "blocked_by_payload_issue_inflight_admission_blocked_canary:"
            f"{self.payload_issue_inflight_admission_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue scheduler-dispatch canary status mismatch")
        if (
            self.payload_issue_scheduler_dispatch_schema
            != "payload_cache_runtime_payload_issue_scheduler_dispatch_v1"
        ):
            raise ValueError("payload issue scheduler-dispatch schema mismatch")
        for field_name in (
        ...
        for field_name in ("scheduler_dispatch_allowed", "scheduler_dispatch_enqueued"):
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue scheduler-dispatch requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue scheduler-dispatch decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue scheduler-dispatch block reason mismatch")
        if (
            self.execution_mode
            != "payload_cache_live_runtime_adapter_payload_issue_scheduler_dispatch_blocked_canary"
        ):
            raise ValueError("payload issue scheduler-dispatch execution mode mismatch")
        for field_name in (
            "live_payload_runtime_enabled",
            "payload_transfer_runtime_enabled",
            "payload_deref_allowed",
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueCommandPacketDryRun:
    """Future payload issue command-packet shape that is never submitted."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_scheduler_dispatch_blocked_canary: bool
    payload_issue_scheduler_dispatch_status: str
    payload_issue_command_packet_schema: str
    payload_issue_command_packet_created: bool
    payload_issue_scheduler_dispatch_consumed: bool
    command_packet_shape_checked: bool
    command_packet_submitted: bool
    command_packet_executed: bool
    request_source: str
    request_layer_idx: int
    request_expert_idx: int
    requested_payload_bytes: int
    ...
    decision: str = "blocked"
    block_reason: str = "payload_transfer_disabled"
    execution_mode: str = "payload_cache_live_runtime_adapter_payload_issue_command_packet_dry_run"
    live_payload_runtime_enabled: bool = False
    payload_transfer_runtime_enabled: bool = False
    payload_deref_allowed: bool = False
    payload_deref_runtime_allowed: bool = False
    ...
    def __post_init__(self) -> None:
        if self.present is not True:
            raise ValueError("payload issue command-packet dry-run must be present")
        if self.stage != "payload_cache_live_runtime_adapter_payload_issue_command_packet_dry_run":
            raise ValueError("payload issue command-packet dry-run stage mismatch")
        if self.consumes_payload_issue_scheduler_dispatch_blocked_canary is not True:
            raise ValueError("command packet must consume scheduler-dispatch canary")
        if (
            not isinstance(self.payload_issue_scheduler_dispatch_status, str)
            or not self.payload_issue_scheduler_dispatch_status
        ):
            raise TypeError("payload_issue_scheduler_dispatch_status must be nonempty")
        if (
            self.payload_issue_scheduler_dispatch_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_SCHEDULER_DISPATCH_BLOCKED_CANARY_STATUS
        ):
            raise ValueError(
                "payload issue command-packet upstream ancestry status mismatch",
            )
        expected_status = (
            "blocked_by_payload_issue_scheduler_dispatch_blocked_canary:"
            f"{self.payload_issue_scheduler_dispatch_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue command-packet dry-run status mismatch")
        if (
            self.payload_issue_command_packet_schema
            != "payload_cache_runtime_payload_issue_command_packet_v1"
        ):
            raise ValueError("payload issue command-packet schema mismatch")
        for field_name in (
        ...
        for field_name in ("command_packet_submitted", "command_packet_executed"):
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue command packet requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue command-packet decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue command-packet block reason mismatch")
        if (
            self.execution_mode
            != "payload_cache_live_runtime_adapter_payload_issue_command_packet_dry_run"
        ):
            raise ValueError("payload issue command-packet execution mode mismatch")
        for field_name in (
            "live_payload_runtime_enabled",
            "payload_transfer_runtime_enabled",
            "payload_deref_allowed",
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueTransportEnqueueBlockedCanary:
    """Transport enqueue canary that rejects a command packet before copy work."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_command_packet_dry_run: bool
    payload_issue_command_packet_status: str
    payload_issue_transport_enqueue_schema: str
    payload_issue_transport_enqueue_canary_created: bool
    payload_issue_command_packet_consumed: bool
    transport_enqueue_checked: bool
    transport_enqueue_rejected: bool
    transport_enqueue_allowed: bool
    transport_work_enqueued: bool
    request_source: str
    request_layer_idx: int
    request_expert_idx: int
    requested_payload_bytes: int
    ...
    def __post_init__(self) -> None:
        ...
        if self.consumes_payload_issue_command_packet_dry_run is not True:
            raise ValueError("transport enqueue must consume command-packet dry-run")
        if (
            not isinstance(self.payload_issue_command_packet_status, str)
            or not self.payload_issue_command_packet_status
        ):
            raise TypeError("payload_issue_command_packet_status must be nonempty")
        if (
            self.payload_issue_command_packet_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_COMMAND_PACKET_DRY_RUN_STATUS
        ):
            raise ValueError(
                "payload issue transport-enqueue upstream ancestry status mismatch",
            )
        expected_status = (
            "blocked_by_payload_issue_command_packet_dry_run:"
            f"{self.payload_issue_command_packet_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue transport-enqueue canary status mismatch")
        if (
            self.payload_issue_transport_enqueue_schema
        ...
        for field_name in ("transport_enqueue_allowed", "transport_work_enqueued"):
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue transport enqueue requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue transport-enqueue decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorDryRun:
    """Future payload copy descriptor shape that is never submitted."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_transport_worker_dispatch_blocked_canary: bool
    payload_issue_transport_worker_dispatch_status: str
    payload_issue_copy_descriptor_schema: str
    payload_issue_copy_descriptor_created: bool
    payload_issue_transport_worker_dispatch_consumed: bool
    copy_descriptor_shape_checked: bool
    ...
    def __post_init__(self) -> None:
        if self.present is not True:
            raise ValueError("payload issue copy-descriptor dry-run must be present")
        if self.stage != "payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_dry_run":
            raise ValueError("payload issue copy-descriptor dry-run stage mismatch")
        if self.consumes_payload_issue_transport_worker_dispatch_blocked_canary is not True:
            raise ValueError("copy descriptor must consume transport-worker dispatch canary")
        if (
            not isinstance(self.payload_issue_transport_worker_dispatch_status, str)
            or not self.payload_issue_transport_worker_dispatch_status
        ):
            raise TypeError("payload_issue_transport_worker_dispatch_status must be nonempty")
        if (
            self.payload_issue_transport_worker_dispatch_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_TRANSPORT_WORKER_DISPATCH_BLOCKED_CANARY_STATUS
        ):
            raise ValueError(
                "payload issue copy-descriptor upstream ancestry status mismatch",
            )
        expected_status = (
            "blocked_by_payload_issue_transport_worker_dispatch_blocked_canary:"
            f"{self.payload_issue_transport_worker_dispatch_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue copy-descriptor dry-run status mismatch")
        if (
            self.payload_issue_copy_descriptor_schema
            != "payload_cache_runtime_payload_issue_copy_descriptor_v1"
        ):
            raise ValueError("payload issue copy-descriptor schema mismatch")
        for field_name in (
        ...
        for field_name in ("copy_descriptor_submitted", "copy_descriptor_executed"):
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue copy descriptor requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue copy-descriptor decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue copy-descriptor block reason mismatch")
        if (
            self.execution_mode
            != "payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_dry_run"
        ):
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorSubmitBlockedCanary:
    """Copy descriptor submit canary that rejects descriptor submission."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_copy_descriptor_dry_run: bool
    payload_issue_copy_descriptor_status: str
    payload_issue_copy_descriptor_submit_schema: str
    payload_issue_copy_descriptor_submit_canary_created: bool
    payload_issue_copy_descriptor_consumed: bool
    copy_descriptor_submit_checked: bool
    copy_descriptor_submit_rejected: bool
    copy_descriptor_submit_allowed: bool
    copy_descriptor_submitted: bool
    copy_descriptor_executed: bool
    request_source: str
    request_layer_idx: int
    ...
    block_reason: str = "payload_transfer_disabled"
    execution_mode: str = (
        "payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_submit_blocked_canary"
    )
    live_payload_runtime_enabled: bool = False
    payload_transfer_runtime_enabled: bool = False
    payload_deref_allowed: bool = False
    ...
    def __post_init__(self) -> None:
        if self.present is not True:
            raise ValueError("payload issue copy-descriptor submit canary must be present")
        if (
            self.stage
            != "payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_submit_blocked_canary"
        ):
            raise ValueError("payload issue copy-descriptor submit canary stage mismatch")
        if self.consumes_payload_issue_copy_descriptor_dry_run is not True:
            raise ValueError("copy descriptor submit must consume copy-descriptor dry-run")
        if (
            not isinstance(self.payload_issue_copy_descriptor_status, str)
            or not self.payload_issue_copy_descriptor_status
        ):
            raise TypeError("payload_issue_copy_descriptor_status must be nonempty")
        if (
            self.payload_issue_copy_descriptor_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_DRY_RUN_STATUS
        ):
            raise ValueError(
                "payload issue copy-descriptor submit upstream ancestry status mismatch",
            )
        expected_status = (
            "blocked_by_payload_issue_copy_descriptor_dry_run:"
            f"{self.payload_issue_copy_descriptor_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue copy-descriptor submit status mismatch")
        if (
            self.payload_issue_copy_descriptor_submit_schema
            != "payload_cache_runtime_payload_issue_copy_descriptor_submit_v1"
        ):
            raise ValueError("payload issue copy-descriptor submit schema mismatch")
        for field_name in (
        ...
        for field_name in (
            ...
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue copy descriptor submit requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue copy-descriptor submit decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue copy-descriptor submit block reason mismatch")
        if (
            self.execution_mode
            != "payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_submit_blocked_canary"
        ):
            raise ValueError("payload issue copy-descriptor submit execution mode mismatch")
        for field_name in (
            "live_payload_runtime_enabled",
            "payload_transfer_runtime_enabled",
            "payload_deref_allowed",
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorDispatchBlockedCanary:
    """Copy descriptor dispatch canary that rejects descriptor execution dispatch."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_copy_descriptor_submit_blocked_canary: bool
    payload_issue_copy_descriptor_submit_status: str
    payload_issue_copy_descriptor_dispatch_schema: str
    payload_issue_copy_descriptor_dispatch_canary_created: bool
    payload_issue_copy_descriptor_submit_consumed: bool
    copy_descriptor_dispatch_checked: bool
    copy_descriptor_dispatch_rejected: bool
    copy_descriptor_dispatch_allowed: bool
    ...
    block_reason: str = "payload_transfer_disabled"
    execution_mode: str = (
        "payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_dispatch_blocked_canary"
    )
    live_payload_runtime_enabled: bool = False
    payload_transfer_runtime_enabled: bool = False
    payload_deref_allowed: bool = False
    ...
    def __post_init__(self) -> None:
        if self.present is not True:
            raise ValueError("payload issue copy-descriptor dispatch canary must be present")
        if (
            self.stage
            != "payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_dispatch_blocked_canary"
        ):
            raise ValueError("payload issue copy-descriptor dispatch canary stage mismatch")
        if self.consumes_payload_issue_copy_descriptor_submit_blocked_canary is not True:
            raise ValueError("copy descriptor dispatch must consume submit-blocked canary")
        if (
            not isinstance(self.payload_issue_copy_descriptor_submit_status, str)
            or not self.payload_issue_copy_descriptor_submit_status
        ):
            raise TypeError("payload_issue_copy_descriptor_submit_status must be nonempty")
        if (
            self.payload_issue_copy_descriptor_submit_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_SUBMIT_BLOCKED_CANARY_STATUS
        ):
            raise ValueError(
                "payload issue copy-descriptor dispatch upstream ancestry status mismatch",
            )
        expected_status = (
            "blocked_by_payload_issue_copy_descriptor_submit_blocked_canary:"
            f"{self.payload_issue_copy_descriptor_submit_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue copy-descriptor dispatch status mismatch")
        if (
            self.payload_issue_copy_descriptor_dispatch_schema
            != "payload_cache_runtime_payload_issue_copy_descriptor_dispatch_v1"
        ):
            raise ValueError("payload issue copy-descriptor dispatch schema mismatch")
        for field_name in (
            "payload_issue_copy_descriptor_dispatch_canary_created",
            "payload_issue_copy_descriptor_submit_consumed",
        ...
        for field_name in (
            ...
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue copy descriptor dispatch requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue copy-descriptor dispatch decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue copy-descriptor dispatch block reason mismatch")
        if (
            self.execution_mode
            != "payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_dispatch_blocked_canary"
        ):
            raise ValueError("payload issue copy-descriptor dispatch execution mode mismatch")
        for field_name in (
            "live_payload_runtime_enabled",
            "payload_transfer_runtime_enabled",
            "payload_deref_allowed",
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorExecutionBlockedCanary:
    """Copy descriptor execution canary that rejects DMA/copy execution."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_copy_descriptor_dispatch_blocked_canary: bool
    payload_issue_copy_descriptor_dispatch_status: str
    payload_issue_copy_descriptor_execution_schema: str
    payload_issue_copy_descriptor_execution_canary_created: bool
    payload_issue_copy_descriptor_dispatch_consumed: bool
    copy_descriptor_execution_checked: bool
    copy_descriptor_execution_rejected: bool
    copy_descriptor_execution_allowed: bool
    copy_descriptor_dispatched: bool
    copy_descriptor_submitted: bool
    copy_descriptor_executed: bool
    request_source: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.consumes_payload_issue_copy_descriptor_dispatch_blocked_canary is not True:
            raise ValueError("copy descriptor execution must consume dispatch-blocked canary")
        if (
            not isinstance(self.payload_issue_copy_descriptor_dispatch_status, str)
            or not self.payload_issue_copy_descriptor_dispatch_status
        ):
            raise TypeError("payload_issue_copy_descriptor_dispatch_status must be nonempty")
        if (
            self.payload_issue_copy_descriptor_dispatch_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_DISPATCH_BLOCKED_CANARY_STATUS
        ):
            raise ValueError(
                "payload issue copy-descriptor execution upstream ancestry status mismatch",
            )
        expected_status = (
            "blocked_by_payload_issue_copy_descriptor_dispatch_blocked_canary:"
            f"{self.payload_issue_copy_descriptor_dispatch_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue copy-descriptor execution status mismatch")
        if (
            self.payload_issue_copy_descriptor_execution_schema
        ...
        for field_name in (
            ...
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue copy descriptor execution requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue copy-descriptor execution decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueReadyCreditBlockedCanary:
    """Ready-credit canary that rejects payload readiness publication."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_copy_completion_blocked_canary: bool
    payload_issue_copy_completion_status: str
    payload_issue_ready_credit_schema: str
    payload_issue_ready_credit_canary_created: bool
    payload_issue_copy_completion_consumed: bool
    ready_credit_checked: bool
    ready_credit_rejected: bool
    ready_credit_allowed: bool
    ...
    block_reason: str = "payload_transfer_disabled"
    execution_mode: str = (
        "payload_cache_live_runtime_adapter_payload_issue_ready_credit_blocked_canary"
    )
    live_payload_runtime_enabled: bool = False
    payload_transfer_runtime_enabled: bool = False
    payload_deref_allowed: bool = False
    ...
    def __post_init__(self) -> None:
        if self.present is not True:
            raise ValueError("payload issue ready-credit canary must be present")
        if (
            self.stage
            != "payload_cache_live_runtime_adapter_payload_issue_ready_credit_blocked_canary"
        ):
            raise ValueError("payload issue ready-credit canary stage mismatch")
        if self.consumes_payload_issue_copy_completion_blocked_canary is not True:
            raise ValueError("ready credit must consume copy-completion blocked canary")
        if (
            not isinstance(self.payload_issue_copy_completion_status, str)
            or not self.payload_issue_copy_completion_status
        ):
            raise TypeError("payload_issue_copy_completion_status must be nonempty")
        if (
            self.payload_issue_copy_completion_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_COMPLETION_BLOCKED_CANARY_STATUS
        ):
            raise ValueError("payload issue ready-credit upstream ancestry status mismatch")
        expected_status = (
            "blocked_by_payload_issue_copy_completion_blocked_canary:"
            f"{self.payload_issue_copy_completion_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue ready-credit status mismatch")
        if (
            self.payload_issue_ready_credit_schema
            != "payload_cache_runtime_payload_issue_ready_credit_v1"
        ):
            raise ValueError("payload issue ready-credit schema mismatch")
        for field_name in (
            "payload_issue_ready_credit_canary_created",
            "payload_issue_copy_completion_consumed",
        ...
        for field_name in (
            ...
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue ready credit requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue ready-credit decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue ready-credit block reason mismatch")
        if (
            self.execution_mode
            != "payload_cache_live_runtime_adapter_payload_issue_ready_credit_blocked_canary"
        ):
            raise ValueError("payload issue ready-credit mode mismatch")
        for field_name in (
            "live_payload_runtime_enabled",
            "payload_transfer_runtime_enabled",
            "payload_deref_allowed",
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssueResidencyUpdateBlockedCanary:
    """Residency-update canary that rejects payload cache state mutation."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_ready_credit_blocked_canary: bool
    payload_issue_ready_credit_status: str
    payload_issue_residency_update_schema: str
    payload_issue_residency_update_canary_created: bool
    payload_issue_ready_credit_consumed: bool
    residency_update_checked: bool
    residency_update_rejected: bool
    residency_update_allowed: bool
    residency_updated: bool
    ...
    def __post_init__(self) -> None:
        if self.present is not True:
            raise ValueError("payload issue residency-update canary must be present")
        if (
            self.stage
            != "payload_cache_live_runtime_adapter_payload_issue_residency_update_blocked_canary"
        ):
            raise ValueError("payload issue residency-update canary stage mismatch")
        if self.consumes_payload_issue_ready_credit_blocked_canary is not True:
            raise ValueError("residency update must consume ready-credit blocked canary")
        if (
            not isinstance(self.payload_issue_ready_credit_status, str)
            or not self.payload_issue_ready_credit_status
        ):
            raise TypeError("payload_issue_ready_credit_status must be nonempty")
        if (
            self.payload_issue_ready_credit_status
            != _SOURCE_BOUND_PAYLOAD_ISSUE_READY_CREDIT_BLOCKED_CANARY_STATUS
        ):
            raise ValueError("payload issue residency-update upstream ancestry status mismatch")
        expected_status = (
            "blocked_by_payload_issue_ready_credit_blocked_canary:"
            f"{self.payload_issue_ready_credit_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue residency-update status mismatch")
        if (
            self.payload_issue_residency_update_schema
            != "payload_cache_runtime_payload_issue_residency_update_v1"
        ):
            raise ValueError("payload issue residency-update schema mismatch")
        for field_name in (
            "payload_issue_residency_update_canary_created",
            "payload_issue_ready_credit_consumed",
            "residency_update_checked",
            "residency_update_rejected",
        ):
            if getattr(self, field_name) is not True:
                raise ValueError(f"{field_name} must be true")
        for field_name in (
            ...
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue residency update requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue residency-update decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
            raise ValueError("payload issue residency-update block reason mismatch")
        if (
            self.execution_mode
            != "payload_cache_live_runtime_adapter_payload_issue_residency_update_blocked_canary"
        ):
            raise ValueError("payload issue residency-update mode mismatch")
        for field_name in (
            "live_payload_runtime_enabled",
            "payload_transfer_runtime_enabled",
            "payload_deref_allowed",
...
@dataclass(frozen=True)
class PayloadCacheLiveRuntimeAdapterPayloadIssuePayloadDerefBlockedCanary:
    """Payload dereference canary that rejects consumer-visible payload access."""

    present: bool
    stage: str
    status: str
    consumes_payload_issue_residency_update_blocked_canary: bool
    payload_issue_residency_update_status: str
    payload_issue_payload_deref_schema: str
    payload_issue_payload_deref_canary_created: bool
    payload_issue_residency_update_consumed: bool
    payload_deref_checked: bool
    payload_deref_rejected: bool
    payload_deref_allowed_for_resident_payload: bool
    payload_deref_attempted: bool
    payload_handle_deref_checked: bool
    payload_handle_deref_rejected: bool
    ...
    def __post_init__(self) -> None:
        ...
        if self.consumes_payload_issue_residency_update_blocked_canary is not True:
            raise ValueError("payload deref must consume residency-update blocked canary")
        if (
            not isinstance(self.payload_issue_residency_update_status, str)
            or not self.payload_issue_residency_update_status
        ):
            raise TypeError("payload_issue_residency_update_status must be nonempty")
        if (
            self.payload_issue_residency_update_status
        ...
        expected_status = (
            ...
            f"{self.payload_issue_residency_update_status}"
        )
        if self.status != expected_status:
            raise ValueError("payload issue payload-deref status mismatch")
        if (
            self.payload_issue_payload_deref_schema
        ...
        for field_name in (
            ...
            if getattr(self, field_name) is not False:
                raise ValueError(f"{field_name} must remain disabled")
        if self.request_source != "queue_budget_first_model_passing_cell":
            raise ValueError("payload issue payload deref requires a source-bound request")
        for field_name in (
            "request_layer_idx",
            "request_expert_idx",
            "requested_payload_bytes",
        ...
        for field_name in (
            ...
            if value != 0:
                raise ValueError(f"{field_name} must remain zero")
        if self.decision != "blocked":
            raise ValueError("payload issue payload-deref decision must stay blocked")
        if self.block_reason != "payload_transfer_disabled":
...
def build_payload_cache_live_runtime_adapter_payload_transfer_toggle_disabled_canary(
    ...
    """Construct a disabled payload-transfer toggle after payloadless adapter gate."""
    ...
    if payload_issue_rejected is not True:
        raise ValueError("disabled payload transfer toggle did not reject issue")

    return PayloadCacheLiveRuntimeAdapterPayloadTransferToggleDisabledCanary(
        present=True,
        ...
        payload_transfer_toggle_created=bool(
            snapshot_payload["payload_transfer_toggle_created"],
        ),
        payload_issue_rejected=bool(snapshot_payload["payload_issue_rejected"]),
        payloadless_live_adapter_created=bool(canary.payloadless_live_adapter_created),
        payloadless_live_operations_ran=bool(canary.payloadless_live_operations_ran),
        live_adapter_instance_created=bool(canary.live_adapter_instance_created),
        ...
        queue_wait_us=float(canary.queue_wait_us),
        queue_max_delay_us=float(canary.queue_max_delay_us),
        shifted_issue_accounting_enabled=bool(canary.shifted_issue_accounting_enabled),
        shifted_issue_accounted_packet_count=int(
            canary.shifted_issue_accounted_packet_count,
        ),
        shifted_issue_unique_issue_key_count=int(
...
_SOURCE_BOUND_PAYLOAD_ISSUE_PLAN_DRY_RUN_STATUS = (
    "blocked_by_payload_issue_request_blocked_canary:"
    f"{_SOURCE_BOUND_PAYLOAD_ISSUE_REQUEST_BLOCKED_STATUS}"
)

_SOURCE_BOUND_PAYLOAD_ISSUE_EXECUTOR_DRY_RUN_STATUS = (
    "blocked_by_payload_issue_plan_dry_run:"
    f"{_SOURCE_BOUND_PAYLOAD_ISSUE_PLAN_DRY_RUN_STATUS}"
)
...
_SOURCE_BOUND_PAYLOAD_ISSUE_INFLIGHT_ADMISSION_BLOCKED_CANARY_STATUS = (
    "blocked_by_payload_issue_queue_submit_blocked_canary:"
    f"{_SOURCE_BOUND_PAYLOAD_ISSUE_QUEUE_SUBMIT_BLOCKED_CANARY_STATUS}"
)

_SOURCE_BOUND_PAYLOAD_ISSUE_SCHEDULER_DISPATCH_BLOCKED_CANARY_STATUS = (
    "blocked_by_payload_issue_inflight_admission_blocked_canary:"
    f"{_SOURCE_BOUND_PAYLOAD_ISSUE_INFLIGHT_ADMISSION_BLOCKED_CANARY_STATUS}"
)
...
_SOURCE_BOUND_PAYLOAD_ISSUE_TRANSPORT_WORKER_DISPATCH_BLOCKED_CANARY_STATUS = (
    "blocked_by_payload_issue_transport_enqueue_blocked_canary:"
    f"{_SOURCE_BOUND_PAYLOAD_ISSUE_TRANSPORT_ENQUEUE_BLOCKED_CANARY_STATUS}"
)

_SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_DRY_RUN_STATUS = (
    "blocked_by_payload_issue_transport_worker_dispatch_blocked_canary:"
    f"{_SOURCE_BOUND_PAYLOAD_ISSUE_TRANSPORT_WORKER_DISPATCH_BLOCKED_CANARY_STATUS}"
)

_SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_SUBMIT_BLOCKED_CANARY_STATUS = (
    "blocked_by_payload_issue_copy_descriptor_dry_run:"
    f"{_SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_DRY_RUN_STATUS}"
)
...
_SOURCE_BOUND_PAYLOAD_ISSUE_COPY_COMPLETION_BLOCKED_CANARY_STATUS = (
    "blocked_by_payload_issue_copy_descriptor_execution_blocked_canary:"
    f"{_SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_EXECUTION_BLOCKED_CANARY_STATUS}"
)
_SOURCE_BOUND_PAYLOAD_ISSUE_READY_CREDIT_BLOCKED_CANARY_STATUS = (
    "blocked_by_payload_issue_copy_completion_blocked_canary:"
    f"{_SOURCE_BOUND_PAYLOAD_ISSUE_COPY_COMPLETION_BLOCKED_CANARY_STATUS}"
)
_SOURCE_BOUND_PAYLOAD_ISSUE_RESIDENCY_UPDATE_BLOCKED_CANARY_STATUS = (
    "blocked_by_payload_issue_ready_credit_blocked_canary:"
...
def build_payload_cache_live_runtime_adapter_payload_issue_plan_dry_run(
    ...
    """Build the next payload issue plan object while transfer remains disabled."""

    if not isinstance(canary, PayloadCacheLiveRuntimeAdapterPayloadIssueRequestBlockedCanary):
        raise TypeError(
            "canary must be a PayloadCacheLiveRuntimeAdapterPayloadIssueRequestBlockedCanary",
        )
    if canary.present is not True:
        raise ValueError("payload issue request canary must be present")
    if (
        ...
        raise ValueError("payload issue request canary stage mismatch")
    if canary.consumes_payload_transfer_toggle_disabled_canary is not True:
        raise ValueError("payload issue request must consume transfer toggle canary")
    if (
        not isinstance(canary.payload_transfer_toggle_disabled_canary_status, str)
    ...
    expected_status = (
        "blocked_by_payload_transfer_toggle_disabled_canary:"
        f"{canary.payload_transfer_toggle_disabled_canary_status}"
    )
    if canary.status != expected_status:
        raise ValueError("payload issue request canary status mismatch")
    if (
        canary.payload_issue_request_schema
        != "payload_cache_runtime_payload_issue_request_v1"
    ):
        raise ValueError("payload issue request schema mismatch")
    if canary.payload_issue_request_created is not True:
        raise ValueError("payload issue request must be created")
    if canary.payload_issue_rejected is not True:
        raise ValueError("payload issue request must be rejected")
    if canary.decision != "blocked":
        raise ValueError("payload issue request canary must stay blocked")
    if (
        canary.block_reason
        != "live_runtime_adapter_payload_issue_request_blocked_canary_only"
    ):
        raise ValueError("payload issue request canary block reason mismatch")
    if (
        canary.execution_mode
        != "payload_cache_live_runtime_adapter_payload_issue_request_blocked_canary_payloadless"
    ):
        raise ValueError("payload issue request canary execution mode mismatch")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if canary.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if not isinstance(canary.request_source, str) or not canary.request_source:
        raise TypeError("request_source must be nonempty")
    if canary.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue plan requires a source-bound queue-budget request")
    if not isinstance(canary.source_queue_deadline_us, (int, float)) or isinstance(
        canary.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if canary.source_queue_deadline_us < 0.0:
        raise ValueError("source_queue_deadline_us must be non-negative")
    if canary.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if canary.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if canary.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if canary.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if canary.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in ("issued_payload_count", "payload_bytes"):
        if getattr(canary, field_name) != 0:
            raise ValueError(f"payload issue request {field_name} must be zero")
    for field_name in (
        "live_payload_runtime_enabled",
        "payload_transfer_runtime_enabled",
        "payload_deref_allowed",
        ...
        if getattr(canary, field_name) is not False:
            raise ValueError(f"payload issue request {field_name} enabled")

    return PayloadCacheLiveRuntimeAdapterPayloadIssuePlanDryRun(
        present=True,
        stage="payload_cache_live_runtime_adapter_payload_issue_plan_dry_run",
        status=f"blocked_by_payload_issue_request_blocked_canary:{canary.status}",
        consumes_payload_issue_request_blocked_canary=True,
        payload_issue_request_blocked_canary_status=str(canary.status),
        request_source=str(canary.request_source),
        request_layer_idx=int(canary.request_layer_idx),
        request_expert_idx=int(canary.request_expert_idx),
        requested_payload_bytes=int(canary.requested_payload_bytes),
        source_issue_packet_count=int(canary.source_issue_packet_count),
        source_issue_unique_key_count=int(canary.source_issue_unique_key_count),
...
def build_payload_cache_live_runtime_adapter_payload_issue_queue_entry_dry_run(
    executor: PayloadCacheLiveRuntimeAdapterPayloadIssueExecutorDryRun,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueQueueEntryDryRun:
    """Build a payload issue queue-entry shape without enqueueing payload work."""

    if not isinstance(executor, PayloadCacheLiveRuntimeAdapterPayloadIssueExecutorDryRun):
        raise TypeError(
            "executor must be a PayloadCacheLiveRuntimeAdapterPayloadIssueExecutorDryRun",
        )
    if executor.present is not True:
        raise ValueError("payload issue executor dry-run must be present")
    if (
        executor.stage
        != "payload_cache_live_runtime_adapter_payload_issue_executor_dry_run"
        ...
        raise ValueError("payload issue executor dry-run stage mismatch")
    if executor.consumes_payload_issue_plan_dry_run is not True:
        raise ValueError("payload issue executor must consume plan dry-run")
    if (
        executor.payload_issue_plan_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_PLAN_DRY_RUN_STATUS
    ):
        raise ValueError("payload issue executor upstream ancestry status chain mismatch")
    if executor.status != _SOURCE_BOUND_PAYLOAD_ISSUE_EXECUTOR_DRY_RUN_STATUS:
        raise ValueError("payload issue executor dry-run status mismatch")
    if executor.payload_issue_executor_schema != "payload_cache_runtime_payload_issue_executor_v1":
        raise ValueError("payload issue executor schema mismatch")
    if executor.payload_issue_executor_created is not True:
        raise ValueError("payload issue executor must be created")
    if executor.payload_issue_plan_consumed is not True:
        raise ValueError("payload issue executor must consume plan")
    if executor.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue queue-entry requires a source-bound executor")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        value = getattr(executor, field_name)
        if not isinstance(value, int) or isinstance(value, bool):
            raise TypeError(f"{field_name} must be an integer")
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if executor.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if executor.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if executor.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if executor.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if executor.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if not isinstance(executor.source_queue_deadline_us, (int, float)) or isinstance(
        executor.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if executor.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in (
        "planned_issue_count",
        "scheduled_issue_count",
        ...
        if getattr(executor, field_name) != 0:
            raise ValueError(f"payload issue executor {field_name} must be zero")
    if executor.decision != "blocked":
        raise ValueError("payload issue executor dry-run must stay blocked")
    if executor.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue executor dry-run block reason mismatch")
    if (
    ...
    for field_name in (
        ...
        if getattr(executor, field_name) is not False:
            raise ValueError(f"payload issue executor {field_name} enabled")

    return PayloadCacheLiveRuntimeAdapterPayloadIssueQueueEntryDryRun(
        present=True,
        ...
        payload_issue_executor_status=str(executor.status),
        ...
        request_source=str(executor.request_source),
        request_layer_idx=int(executor.request_layer_idx),
        request_expert_idx=int(executor.request_expert_idx),
        requested_payload_bytes=int(executor.requested_payload_bytes),
        source_issue_packet_count=int(executor.source_issue_packet_count),
        source_issue_unique_key_count=int(executor.source_issue_unique_key_count),
        source_queue_budget_capacity=int(executor.source_queue_budget_capacity),
        source_issue_lead_tokens=int(executor.source_issue_lead_tokens),
        source_queue_deadline_us=float(executor.source_queue_deadline_us),
    )


def build_payload_cache_live_runtime_adapter_payload_issue_queue_submit_blocked_canary(
    entry: PayloadCacheLiveRuntimeAdapterPayloadIssueQueueEntryDryRun,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueQueueSubmitBlockedCanary:
    """Build a blocked submit-gate canary from a queue-entry shape."""

    if not isinstance(entry, PayloadCacheLiveRuntimeAdapterPayloadIssueQueueEntryDryRun):
        raise TypeError(
            "entry must be a PayloadCacheLiveRuntimeAdapterPayloadIssueQueueEntryDryRun",
        )
    if entry.present is not True:
        raise ValueError("payload issue queue-entry dry-run must be present")
    if entry.stage != "payload_cache_live_runtime_adapter_payload_issue_queue_entry_dry_run":
        raise ValueError("payload issue queue-entry dry-run stage mismatch")
    if entry.consumes_payload_issue_executor_dry_run is not True:
        raise ValueError("payload issue queue-entry must consume executor dry-run")
    if entry.payload_issue_executor_status != _SOURCE_BOUND_PAYLOAD_ISSUE_EXECUTOR_DRY_RUN_STATUS:
        raise ValueError("payload issue queue-entry upstream ancestry status chain mismatch")
    if entry.status != _SOURCE_BOUND_PAYLOAD_ISSUE_QUEUE_ENTRY_DRY_RUN_STATUS:
        raise ValueError("payload issue queue-entry dry-run status mismatch")
    if (
        entry.payload_issue_queue_entry_schema
        != "payload_cache_runtime_payload_issue_queue_entry_v1"
    ):
        raise ValueError("payload issue queue-entry schema mismatch")
    for field_name in (
        ...
        if getattr(entry, field_name) is not True:
            raise ValueError(f"payload issue queue-entry {field_name} must be true")
    for field_name in ("queue_entry_enqueued", "queue_submit_allowed"):
        if getattr(entry, field_name) is not False:
            raise ValueError(f"payload issue queue-entry {field_name} enabled")
    if entry.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue queue-submit requires a source-bound entry")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if entry.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if entry.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if entry.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if entry.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if entry.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if not isinstance(entry.source_queue_deadline_us, (int, float)) or isinstance(
        entry.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if entry.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in (
        "planned_issue_count",
        "scheduled_issue_count",
        ...
        if getattr(entry, field_name) != 0:
            raise ValueError(f"payload issue queue-entry {field_name} must be zero")
    if entry.decision != "blocked":
        raise ValueError("payload issue queue-entry dry-run must stay blocked")
    if entry.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue queue-entry dry-run block reason mismatch")
    if (
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueQueueSubmitBlockedCanary(
        present=True,
        ...
        payload_issue_queue_entry_status=str(entry.status),
        ...
        queue_submit_allowed=False,
        queue_entry_enqueued=False,
        request_source=str(entry.request_source),
        request_layer_idx=int(entry.request_layer_idx),
        request_expert_idx=int(entry.request_expert_idx),
        requested_payload_bytes=int(entry.requested_payload_bytes),
        source_issue_packet_count=int(entry.source_issue_packet_count),
        source_issue_unique_key_count=int(entry.source_issue_unique_key_count),
...
def build_payload_cache_live_runtime_adapter_payload_issue_scheduler_dispatch_blocked_canary(
    admission: PayloadCacheLiveRuntimeAdapterPayloadIssueInflightAdmissionBlockedCanary,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueSchedulerDispatchBlockedCanary:
    """Build a blocked scheduler-dispatch canary from in-flight admission."""
    ...
    if admission.present is not True:
        raise ValueError("payload issue in-flight admission canary must be present")
    if (
        admission.stage
        != "payload_cache_live_runtime_adapter_payload_issue_inflight_admission_blocked_canary"
        ...
        raise ValueError("payload issue in-flight admission canary stage mismatch")
    if admission.consumes_payload_issue_queue_submit_blocked_canary is not True:
        raise ValueError("in-flight admission must consume queue-submit canary")
    if (
        admission.payload_issue_queue_submit_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_QUEUE_SUBMIT_BLOCKED_CANARY_STATUS
    ):
        raise ValueError(
            "payload issue in-flight admission upstream ancestry status chain mismatch",
        )
    if admission.status != _SOURCE_BOUND_PAYLOAD_ISSUE_INFLIGHT_ADMISSION_BLOCKED_CANARY_STATUS:
        raise ValueError("payload issue in-flight admission status mismatch")
    if (
        admission.payload_issue_inflight_admission_schema
        != "payload_cache_runtime_payload_issue_inflight_admission_v1"
    ):
        raise ValueError("payload issue in-flight admission schema mismatch")
    for field_name in (
        ...
        if getattr(admission, field_name) is not True:
            raise ValueError(f"payload issue in-flight admission {field_name} must be true")
    for field_name in ("inflight_admission_allowed", "inflight_queue_enqueued"):
        if getattr(admission, field_name) is not False:
            raise ValueError(f"payload issue in-flight admission {field_name} enabled")
    if admission.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue scheduler-dispatch requires a source-bound admission")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if admission.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if admission.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if admission.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if admission.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if admission.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if not isinstance(admission.source_queue_deadline_us, (int, float)) or isinstance(
        admission.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if admission.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in (
        "planned_issue_count",
        "scheduled_issue_count",
        ...
        if getattr(admission, field_name) != 0:
            raise ValueError(f"payload issue in-flight admission {field_name} must be zero")
    if admission.decision != "blocked":
        raise ValueError("payload issue in-flight admission must stay blocked")
    if admission.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue in-flight admission block reason mismatch")
    if (
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueSchedulerDispatchBlockedCanary(
        present=True,
        stage=(
            "payload_cache_live_runtime_adapter_"
        ...
        payload_issue_inflight_admission_status=str(admission.status),
        ...
        request_source=str(admission.request_source),
        request_layer_idx=int(admission.request_layer_idx),
        request_expert_idx=int(admission.request_expert_idx),
        requested_payload_bytes=int(admission.requested_payload_bytes),
        source_issue_packet_count=int(admission.source_issue_packet_count),
        source_issue_unique_key_count=int(admission.source_issue_unique_key_count),
        source_queue_budget_capacity=int(admission.source_queue_budget_capacity),
        source_issue_lead_tokens=int(admission.source_issue_lead_tokens),
        source_queue_deadline_us=float(admission.source_queue_deadline_us),
    )


def build_payload_cache_live_runtime_adapter_payload_issue_command_packet_dry_run(
    dispatch: PayloadCacheLiveRuntimeAdapterPayloadIssueSchedulerDispatchBlockedCanary,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueCommandPacketDryRun:
    """Build a dry-run payload issue command packet from scheduler dispatch."""
    ...
    if dispatch.present is not True:
        raise ValueError("payload issue scheduler-dispatch canary must be present")
    if (
        dispatch.stage
        != "payload_cache_live_runtime_adapter_payload_issue_scheduler_dispatch_blocked_canary"
        ...
        raise ValueError("payload issue scheduler-dispatch canary stage mismatch")
    if dispatch.consumes_payload_issue_inflight_admission_blocked_canary is not True:
        raise ValueError("scheduler dispatch must consume in-flight admission canary")
    if (
        dispatch.payload_issue_inflight_admission_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_INFLIGHT_ADMISSION_BLOCKED_CANARY_STATUS
    ):
        raise ValueError(
            "payload issue scheduler-dispatch upstream ancestry status chain mismatch",
        )
    if dispatch.status != _SOURCE_BOUND_PAYLOAD_ISSUE_SCHEDULER_DISPATCH_BLOCKED_CANARY_STATUS:
        raise ValueError("payload issue scheduler-dispatch status mismatch")
    if (
        dispatch.payload_issue_scheduler_dispatch_schema
        != "payload_cache_runtime_payload_issue_scheduler_dispatch_v1"
    ):
        raise ValueError("payload issue scheduler-dispatch schema mismatch")
    for field_name in (
        ...
        if getattr(dispatch, field_name) is not True:
            raise ValueError(f"payload issue scheduler-dispatch {field_name} must be true")
    for field_name in ("scheduler_dispatch_allowed", "scheduler_dispatch_enqueued"):
        if getattr(dispatch, field_name) is not False:
            raise ValueError(f"payload issue scheduler-dispatch {field_name} enabled")
    if dispatch.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue command packet requires a source-bound dispatch")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        value = getattr(dispatch, field_name)
        if not isinstance(value, int) or isinstance(value, bool):
            raise TypeError(f"{field_name} must be an integer")
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if dispatch.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if dispatch.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if dispatch.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if dispatch.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if dispatch.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if not isinstance(dispatch.source_queue_deadline_us, (int, float)) or isinstance(
        dispatch.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if dispatch.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in (
        "planned_issue_count",
        "scheduled_issue_count",
        ...
        if getattr(dispatch, field_name) != 0:
            raise ValueError(f"payload issue scheduler-dispatch {field_name} must be zero")
    if dispatch.decision != "blocked":
        raise ValueError("payload issue scheduler-dispatch must stay blocked")
    if dispatch.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue scheduler-dispatch block reason mismatch")
    if (
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueCommandPacketDryRun(
        present=True,
        ...
        payload_issue_scheduler_dispatch_status=str(dispatch.status),
        ...
        request_source=str(dispatch.request_source),
        request_layer_idx=int(dispatch.request_layer_idx),
        request_expert_idx=int(dispatch.request_expert_idx),
        requested_payload_bytes=int(dispatch.requested_payload_bytes),
        source_issue_packet_count=int(dispatch.source_issue_packet_count),
        source_issue_unique_key_count=int(dispatch.source_issue_unique_key_count),
...
def build_payload_cache_live_runtime_adapter_payload_issue_transport_enqueue_blocked_canary(
    packet: PayloadCacheLiveRuntimeAdapterPayloadIssueCommandPacketDryRun,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueTransportEnqueueBlockedCanary:
    """Build a blocked transport-enqueue canary from a command packet."""

    if not isinstance(packet, PayloadCacheLiveRuntimeAdapterPayloadIssueCommandPacketDryRun):
        raise TypeError(
            "packet must be a PayloadCacheLiveRuntimeAdapterPayloadIssueCommandPacketDryRun",
        )
    if packet.present is not True:
        raise ValueError("payload issue command-packet dry-run must be present")
    if packet.stage != "payload_cache_live_runtime_adapter_payload_issue_command_packet_dry_run":
        raise ValueError("payload issue command-packet dry-run stage mismatch")
    if packet.consumes_payload_issue_scheduler_dispatch_blocked_canary is not True:
        raise ValueError("command packet must consume scheduler-dispatch canary")
    if (
        packet.payload_issue_scheduler_dispatch_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_SCHEDULER_DISPATCH_BLOCKED_CANARY_STATUS
    ):
        raise ValueError(
            "payload issue command-packet upstream ancestry status chain mismatch",
        )
    if packet.status != _SOURCE_BOUND_PAYLOAD_ISSUE_COMMAND_PACKET_DRY_RUN_STATUS:
        raise ValueError("payload issue command-packet status mismatch")
    if (
        packet.payload_issue_command_packet_schema
        != "payload_cache_runtime_payload_issue_command_packet_v1"
    ):
        raise ValueError("payload issue command-packet schema mismatch")
    for field_name in (
        ...
        if getattr(packet, field_name) is not True:
            raise ValueError(f"payload issue command-packet {field_name} must be true")
    for field_name in ("command_packet_submitted", "command_packet_executed"):
        if getattr(packet, field_name) is not False:
            raise ValueError(f"payload issue command-packet {field_name} enabled")
    if packet.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue transport enqueue requires a source-bound packet")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if packet.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if packet.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if packet.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if packet.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if packet.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if not isinstance(packet.source_queue_deadline_us, (int, float)) or isinstance(
        packet.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if packet.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in (
        "planned_issue_count",
        "scheduled_issue_count",
        ...
        if getattr(packet, field_name) != 0:
            raise ValueError(f"payload issue command-packet {field_name} must be zero")
    if packet.decision != "blocked":
        raise ValueError("payload issue command-packet must stay blocked")
    if packet.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue command-packet block reason mismatch")
    if (
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueTransportEnqueueBlockedCanary(
        present=True,
        stage=(
            "payload_cache_live_runtime_adapter_"
        ...
        payload_issue_command_packet_status=str(packet.status),
        ...
        request_source=str(packet.request_source),
        request_layer_idx=int(packet.request_layer_idx),
        request_expert_idx=int(packet.request_expert_idx),
        requested_payload_bytes=int(packet.requested_payload_bytes),
        source_issue_packet_count=int(packet.source_issue_packet_count),
        source_issue_unique_key_count=int(packet.source_issue_unique_key_count),
...
def build_payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_dry_run(
    dispatch: PayloadCacheLiveRuntimeAdapterPayloadIssueTransportWorkerDispatchBlockedCanary,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorDryRun:
    """Build a dry-run payload copy descriptor from worker-dispatch rejection."""
    ...
    if (
        ...
        raise ValueError("payload issue transport-worker canary stage mismatch")
    if dispatch.consumes_payload_issue_transport_enqueue_blocked_canary is not True:
        raise ValueError("transport worker dispatch must consume transport-enqueue canary")
    if (
        dispatch.payload_issue_transport_enqueue_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_TRANSPORT_ENQUEUE_BLOCKED_CANARY_STATUS
    ):
        raise ValueError(
            "payload issue transport-worker upstream ancestry status chain mismatch",
        )
    if (
        dispatch.status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_TRANSPORT_WORKER_DISPATCH_BLOCKED_CANARY_STATUS
    ):
        raise ValueError("payload issue transport-worker status mismatch")
    if (
        dispatch.payload_issue_transport_worker_dispatch_schema
        != "payload_cache_runtime_payload_issue_transport_worker_dispatch_v1"
    ):
        raise ValueError("payload issue transport-worker schema mismatch")
    for field_name in (
        ...
        if getattr(dispatch, field_name) is not True:
            raise ValueError(f"payload issue transport-worker {field_name} must be true")
    for field_name in ("transport_worker_dispatch_allowed", "transport_worker_dispatched"):
        if getattr(dispatch, field_name) is not False:
            raise ValueError(f"payload issue transport-worker {field_name} enabled")
    if dispatch.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue copy descriptor requires a source-bound dispatch")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
    ...
    for field_name in (
        ...
        if getattr(dispatch, field_name) != 0:
            raise ValueError(f"payload issue transport-worker {field_name} must be zero")
    if dispatch.decision != "blocked":
        raise ValueError("payload issue transport-worker must stay blocked")
    if dispatch.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue transport-worker block reason mismatch")
    if (
        dispatch.execution_mode
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorDryRun(
        present=True,
        ...
        payload_issue_transport_worker_dispatch_status=str(dispatch.status),
...
def build_payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_submit_blocked_canary(
    descriptor: PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorDryRun,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorSubmitBlockedCanary:
    """Build a blocked copy-descriptor submit canary from descriptor dry-run."""
    ...
    if descriptor.present is not True:
        raise ValueError("payload issue copy descriptor must be present")
    if descriptor.stage != "payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_dry_run":
        raise ValueError("payload issue copy descriptor stage mismatch")
    if descriptor.consumes_payload_issue_transport_worker_dispatch_blocked_canary is not True:
        raise ValueError("copy descriptor must consume transport-worker dispatch canary")
    if (
        descriptor.payload_issue_transport_worker_dispatch_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_TRANSPORT_WORKER_DISPATCH_BLOCKED_CANARY_STATUS
    ):
        raise ValueError("payload issue copy descriptor upstream ancestry status chain mismatch")
    if descriptor.status != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_DRY_RUN_STATUS:
        raise ValueError("payload issue copy descriptor status mismatch")
    if (
        descriptor.payload_issue_copy_descriptor_schema
        != "payload_cache_runtime_payload_issue_copy_descriptor_v1"
    ):
        raise ValueError("payload issue copy descriptor schema mismatch")
    for field_name in (
        ...
        if getattr(descriptor, field_name) is not True:
            raise ValueError(f"payload issue copy descriptor {field_name} must be true")
    for field_name in ("copy_descriptor_submitted", "copy_descriptor_executed"):
        if getattr(descriptor, field_name) is not False:
            raise ValueError(f"payload issue copy descriptor {field_name} enabled")
    if descriptor.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue copy descriptor submit requires a source-bound descriptor")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if descriptor.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if descriptor.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if descriptor.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if descriptor.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if descriptor.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if not isinstance(descriptor.source_queue_deadline_us, (int, float)) or isinstance(
        descriptor.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if descriptor.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in (
        "planned_issue_count",
        "scheduled_issue_count",
        ...
        if getattr(descriptor, field_name) != 0:
            raise ValueError(f"payload issue copy descriptor {field_name} must be zero")
    if descriptor.decision != "blocked":
        raise ValueError("payload issue copy descriptor must stay blocked")
    if descriptor.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue copy descriptor block reason mismatch")
    if (
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorSubmitBlockedCanary(
        present=True,
        stage=(
            "payload_cache_live_runtime_adapter_"
        ...
        payload_issue_copy_descriptor_status=str(descriptor.status),
        ...
        copy_descriptor_submitted=False,
        copy_descriptor_executed=False,
        request_source=str(descriptor.request_source),
        request_layer_idx=int(descriptor.request_layer_idx),
        request_expert_idx=int(descriptor.request_expert_idx),
        requested_payload_bytes=int(descriptor.requested_payload_bytes),
        source_issue_packet_count=int(descriptor.source_issue_packet_count),
        source_issue_unique_key_count=int(descriptor.source_issue_unique_key_count),
        source_queue_budget_capacity=int(descriptor.source_queue_budget_capacity),
        source_issue_lead_tokens=int(descriptor.source_issue_lead_tokens),
        source_queue_deadline_us=float(descriptor.source_queue_deadline_us),
    )


def build_payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_dispatch_blocked_canary(
    submit: PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorSubmitBlockedCanary,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorDispatchBlockedCanary:
    """Build a blocked copy-descriptor dispatch canary from submit rejection."""
    ...
    if (
        ...
        raise ValueError("payload issue copy descriptor submit canary stage mismatch")
    if submit.consumes_payload_issue_copy_descriptor_dry_run is not True:
        raise ValueError("copy descriptor submit must consume copy-descriptor dry-run")
    if (
        submit.payload_issue_copy_descriptor_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_DRY_RUN_STATUS
    ):
        raise ValueError(
            "payload issue copy descriptor submit upstream ancestry status chain mismatch",
        )
    if submit.status != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_SUBMIT_BLOCKED_CANARY_STATUS:
        raise ValueError("payload issue copy descriptor submit status mismatch")
    if (
        submit.payload_issue_copy_descriptor_submit_schema
        != "payload_cache_runtime_payload_issue_copy_descriptor_submit_v1"
    ):
        raise ValueError("payload issue copy descriptor submit schema mismatch")
    for field_name in (
        ...
        if getattr(submit, field_name) is not True:
            raise ValueError(f"payload issue copy descriptor submit {field_name} must be true")
    for field_name in (
        "copy_descriptor_submit_allowed",
        "copy_descriptor_submitted",
        "copy_descriptor_executed",
        ...
        if getattr(submit, field_name) is not False:
            raise ValueError(f"payload issue copy descriptor submit {field_name} enabled")
    if submit.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue copy descriptor dispatch requires a source-bound submit")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
    ...
    for field_name in (
        ...
        if getattr(submit, field_name) != 0:
            raise ValueError(f"payload issue copy descriptor submit {field_name} must be zero")
    if submit.decision != "blocked":
        raise ValueError("payload issue copy descriptor submit must stay blocked")
    if submit.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue copy descriptor submit block reason mismatch")
    if (
        submit.execution_mode
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorDispatchBlockedCanary(
        present=True,
        stage=(
            "payload_cache_live_runtime_adapter_"
        ...
        payload_issue_copy_descriptor_submit_status=str(submit.status),
...
def build_payload_cache_live_runtime_adapter_payload_issue_copy_descriptor_execution_blocked_canary(
    dispatch: PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorDispatchBlockedCanary,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorExecutionBlockedCanary:
    """Build a blocked copy-descriptor execution canary from dispatch rejection."""
    ...
    if (
        ...
        raise ValueError("payload issue copy descriptor dispatch canary stage mismatch")
    if dispatch.consumes_payload_issue_copy_descriptor_submit_blocked_canary is not True:
        raise ValueError("copy descriptor dispatch must consume submit-blocked canary")
    if (
        dispatch.payload_issue_copy_descriptor_submit_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_SUBMIT_BLOCKED_CANARY_STATUS
    ):
        raise ValueError(
            "payload issue copy descriptor dispatch upstream ancestry status chain mismatch",
        )
    if dispatch.status != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_DISPATCH_BLOCKED_CANARY_STATUS:
        raise ValueError("payload issue copy descriptor dispatch status mismatch")
    if (
        dispatch.payload_issue_copy_descriptor_dispatch_schema
        != "payload_cache_runtime_payload_issue_copy_descriptor_dispatch_v1"
    ):
        raise ValueError("payload issue copy descriptor dispatch schema mismatch")
    for field_name in (
        ...
        if getattr(dispatch, field_name) is not True:
            raise ValueError(f"payload issue copy descriptor dispatch {field_name} must be true")
    for field_name in (
        "copy_descriptor_dispatch_allowed",
        "copy_descriptor_dispatched",
        "copy_descriptor_submitted",
        ...
        if getattr(dispatch, field_name) is not False:
            raise ValueError(f"payload issue copy descriptor dispatch {field_name} enabled")
    if dispatch.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue copy descriptor execution requires a source-bound dispatch")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
    ...
    for field_name in (
        ...
        if getattr(dispatch, field_name) != 0:
            raise ValueError(f"payload issue copy descriptor dispatch {field_name} must be zero")
    if dispatch.decision != "blocked":
        raise ValueError("payload issue copy descriptor dispatch must stay blocked")
    if dispatch.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue copy descriptor dispatch block reason mismatch")
    if (
        dispatch.execution_mode
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueCopyDescriptorExecutionBlockedCanary(
        present=True,
        stage=(
            "payload_cache_live_runtime_adapter_"
        ...
        payload_issue_copy_descriptor_dispatch_status=str(dispatch.status),
...
def build_payload_cache_live_runtime_adapter_payload_issue_ready_credit_blocked_canary(
    completion: PayloadCacheLiveRuntimeAdapterPayloadIssueCopyCompletionBlockedCanary,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueReadyCreditBlockedCanary:
    """Build a blocked ready-credit canary from copy-completion rejection."""
    ...
    if not isinstance(
        ...
        raise TypeError(
            "completion must be a "
            "PayloadCacheLiveRuntimeAdapterPayloadIssueCopyCompletionBlockedCanary",
        )
    if completion.present is not True:
        raise ValueError("payload issue copy completion canary must be present")
    if (
        completion.stage
        != "payload_cache_live_runtime_adapter_payload_issue_copy_completion_blocked_canary"
        ...
        raise ValueError("payload issue copy completion canary stage mismatch")
    if completion.consumes_payload_issue_copy_descriptor_execution_blocked_canary is not True:
        raise ValueError("copy completion must consume execution-blocked canary")
    if (
        completion.payload_issue_copy_descriptor_execution_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_DESCRIPTOR_EXECUTION_BLOCKED_CANARY_STATUS
    ):
        raise ValueError("payload issue copy completion upstream ancestry status chain mismatch")
    if completion.status != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_COMPLETION_BLOCKED_CANARY_STATUS:
        raise ValueError("payload issue copy completion status mismatch")
    if (
        completion.payload_issue_copy_completion_schema
        != "payload_cache_runtime_payload_issue_copy_completion_v1"
    ):
        raise ValueError("payload issue copy completion schema mismatch")
    for field_name in (
        ...
        if getattr(completion, field_name) is not True:
            raise ValueError(f"payload issue copy completion {field_name} must be true")
    for field_name in (
        "copy_completion_allowed",
        "copy_completed",
        "copy_descriptor_dispatched",
        ...
        if getattr(completion, field_name) is not False:
            raise ValueError(f"payload issue copy completion {field_name} enabled")
    if completion.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue ready credit requires a source-bound completion")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if completion.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if completion.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if completion.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if completion.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if completion.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if not isinstance(completion.source_queue_deadline_us, (int, float)) or isinstance(
        completion.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if completion.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in (
        "planned_issue_count",
        "scheduled_issue_count",
        ...
        if getattr(completion, field_name) != 0:
            raise ValueError(f"payload issue copy completion {field_name} must be zero")
    if completion.decision != "blocked":
        raise ValueError("payload issue copy completion must stay blocked")
    if completion.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue copy completion block reason mismatch")
    if (
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueReadyCreditBlockedCanary(
        present=True,
        stage=(
            "payload_cache_live_runtime_adapter_"
            "payload_issue_ready_credit_blocked_canary"
        ),
        status=f"blocked_by_payload_issue_copy_completion_blocked_canary:{completion.status}",
        consumes_payload_issue_copy_completion_blocked_canary=True,
        payload_issue_copy_completion_status=str(completion.status),
        payload_issue_ready_credit_schema=(
            "payload_cache_runtime_payload_issue_ready_credit_v1"
        ),
        ...
        copy_descriptor_submitted=False,
        copy_descriptor_executed=False,
        request_source=str(completion.request_source),
        request_layer_idx=int(completion.request_layer_idx),
        request_expert_idx=int(completion.request_expert_idx),
        requested_payload_bytes=int(completion.requested_payload_bytes),
        source_issue_packet_count=int(completion.source_issue_packet_count),
        source_issue_unique_key_count=int(completion.source_issue_unique_key_count),
        source_queue_budget_capacity=int(completion.source_queue_budget_capacity),
        source_issue_lead_tokens=int(completion.source_issue_lead_tokens),
        source_queue_deadline_us=float(completion.source_queue_deadline_us),
    )


def build_payload_cache_live_runtime_adapter_payload_issue_residency_update_blocked_canary(
    ready: PayloadCacheLiveRuntimeAdapterPayloadIssueReadyCreditBlockedCanary,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssueResidencyUpdateBlockedCanary:
    """Build a blocked residency-update canary from ready-credit rejection."""
    ...
    if not isinstance(
        ...
        raise TypeError(
            "ready must be a PayloadCacheLiveRuntimeAdapterPayloadIssueReadyCreditBlockedCanary",
        )
    if ready.present is not True:
        raise ValueError("payload issue ready-credit canary must be present")
    if ready.stage != "payload_cache_live_runtime_adapter_payload_issue_ready_credit_blocked_canary":
        raise ValueError("payload issue ready-credit canary stage mismatch")
    if ready.consumes_payload_issue_copy_completion_blocked_canary is not True:
        raise ValueError("ready credit must consume copy-completion blocked canary")
    if (
        ready.payload_issue_copy_completion_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_COPY_COMPLETION_BLOCKED_CANARY_STATUS
    ):
        raise ValueError("payload issue ready-credit upstream ancestry status chain mismatch")
    if ready.status != _SOURCE_BOUND_PAYLOAD_ISSUE_READY_CREDIT_BLOCKED_CANARY_STATUS:
        raise ValueError("payload issue ready-credit status mismatch")
    if ready.payload_issue_ready_credit_schema != "payload_cache_runtime_payload_issue_ready_credit_v1":
        raise ValueError("payload issue ready-credit schema mismatch")
    for field_name in (
        "payload_issue_ready_credit_canary_created",
        "payload_issue_copy_completion_consumed",
        ...
        if getattr(ready, field_name) is not True:
            raise ValueError(f"payload issue ready-credit {field_name} must be true")
    for field_name in (
        "ready_credit_allowed",
        "ready_credit_granted",
        "ready_before_demand_credit_granted",
        ...
        if getattr(ready, field_name) is not False:
            raise ValueError(f"payload issue ready-credit {field_name} enabled")
    if ready.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue residency update requires a source-bound ready canary")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        value = getattr(ready, field_name)
        if not isinstance(value, int) or isinstance(value, bool):
            raise TypeError(f"{field_name} must be an integer")
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if ready.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if ready.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if ready.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if ready.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if ready.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if not isinstance(ready.source_queue_deadline_us, (int, float)) or isinstance(
        ready.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if ready.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in (
        "planned_issue_count",
        "scheduled_issue_count",
        ...
        if getattr(ready, field_name) != 0:
            raise ValueError(f"payload issue ready-credit {field_name} must be zero")
    if ready.decision != "blocked":
        raise ValueError("payload issue ready-credit must stay blocked")
    if ready.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue ready-credit block reason mismatch")
    if ready.execution_mode != "payload_cache_live_runtime_adapter_payload_issue_ready_credit_blocked_canary":
        raise ValueError("payload issue ready-credit mode mismatch")
    for field_name in (
        "live_payload_runtime_enabled",
        "payload_transfer_runtime_enabled",
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssueResidencyUpdateBlockedCanary(
        present=True,
        stage=(
            "payload_cache_live_runtime_adapter_"
        ...
        payload_issue_ready_credit_status=str(ready.status),
        ...
        real_payload_ready=False,
        copy_completed=False,
        request_source=str(ready.request_source),
        request_layer_idx=int(ready.request_layer_idx),
        request_expert_idx=int(ready.request_expert_idx),
        requested_payload_bytes=int(ready.requested_payload_bytes),
        source_issue_packet_count=int(ready.source_issue_packet_count),
        source_issue_unique_key_count=int(ready.source_issue_unique_key_count),
        source_queue_budget_capacity=int(ready.source_queue_budget_capacity),
        source_issue_lead_tokens=int(ready.source_issue_lead_tokens),
        source_queue_deadline_us=float(ready.source_queue_deadline_us),
    )


def build_payload_cache_live_runtime_adapter_payload_issue_payload_deref_blocked_canary(
    residency: PayloadCacheLiveRuntimeAdapterPayloadIssueResidencyUpdateBlockedCanary,
) -> PayloadCacheLiveRuntimeAdapterPayloadIssuePayloadDerefBlockedCanary:
    """Build a blocked payload deref canary from residency-update rejection."""
    ...
    if residency.present is not True:
        raise ValueError("payload issue residency-update canary must be present")
    if (
        residency.stage
        != "payload_cache_live_runtime_adapter_payload_issue_residency_update_blocked_canary"
        ...
        raise ValueError("payload issue residency-update canary stage mismatch")
    if residency.consumes_payload_issue_ready_credit_blocked_canary is not True:
        raise ValueError("residency update must consume ready-credit blocked canary")
    if (
        residency.payload_issue_ready_credit_status
        != _SOURCE_BOUND_PAYLOAD_ISSUE_READY_CREDIT_BLOCKED_CANARY_STATUS
    ):
        raise ValueError("payload issue residency-update upstream ancestry status mismatch")
    if residency.status != _SOURCE_BOUND_PAYLOAD_ISSUE_RESIDENCY_UPDATE_BLOCKED_CANARY_STATUS:
        raise ValueError("payload issue residency-update status mismatch")
    if (
        residency.payload_issue_residency_update_schema
        != "payload_cache_runtime_payload_issue_residency_update_v1"
    ):
        raise ValueError("payload issue residency-update schema mismatch")
    for field_name in (
        ...
        if getattr(residency, field_name) is not True:
            raise ValueError(f"payload issue residency-update {field_name} must be true")
    for field_name in (
        "residency_update_allowed",
        "residency_updated",
        "payload_marked_resident",
        ...
        if getattr(residency, field_name) is not False:
            raise ValueError(f"payload issue residency-update {field_name} enabled")
    if residency.request_source != "queue_budget_first_model_passing_cell":
        raise ValueError("payload issue payload deref requires a source-bound residency canary")
    for field_name in (
        "request_layer_idx",
        "request_expert_idx",
        "requested_payload_bytes",
        ...
        value = getattr(residency, field_name)
        if not isinstance(value, int) or isinstance(value, bool):
            raise TypeError(f"{field_name} must be an integer")
        if value < 0:
            raise ValueError(f"{field_name} must be non-negative")
    if residency.requested_payload_bytes <= 0:
        raise ValueError("requested_payload_bytes must be positive")
    if residency.source_issue_packet_count <= 0:
        raise ValueError("source_issue_packet_count must be positive")
    if residency.source_issue_unique_key_count <= 0:
        raise ValueError("source_issue_unique_key_count must be positive")
    if residency.source_queue_budget_capacity <= 0:
        raise ValueError("source_queue_budget_capacity must be positive")
    if residency.source_issue_lead_tokens <= 0:
        raise ValueError("source_issue_lead_tokens must be positive")
    if not isinstance(residency.source_queue_deadline_us, (int, float)) or isinstance(
        residency.source_queue_deadline_us,
        bool,
    ):
        raise TypeError("source_queue_deadline_us must be numeric")
    if residency.source_queue_deadline_us <= 0.0:
        raise ValueError("source_queue_deadline_us must be positive")
    for field_name in (
        "planned_issue_count",
        "scheduled_issue_count",
        ...
        if getattr(residency, field_name) != 0:
            raise ValueError(f"payload issue residency-update {field_name} must be zero")
    if residency.decision != "blocked":
        raise ValueError("payload issue residency-update must stay blocked")
    if residency.block_reason != "payload_transfer_disabled":
        raise ValueError("payload issue residency-update block reason mismatch")
    if (
        residency.execution_mode
        != "payload_cache_live_runtime_adapter_payload_issue_residency_update_blocked_canary"
    ):
        raise ValueError("payload issue residency-update mode mismatch")
    for field_name in (
    ...
    return PayloadCacheLiveRuntimeAdapterPayloadIssuePayloadDerefBlockedCanary(
        present=True,
        stage=(
            "payload_cache_live_runtime_adapter_"
            "payload_issue_payload_deref_blocked_canary"
        ),
        status=f"blocked_by_payload_issue_residency_update_blocked_canary:{residency.status}",
        consumes_payload_issue_residency_update_blocked_canary=True,
        payload_issue_residency_update_status=str(residency.status),
        payload_issue_payload_deref_schema=(
            "payload_cache_runtime_payload_issue_payload_deref_v1"
        ),
        payload_issue_payload_deref_canary_created=True,
        payload_issue_residency_update_consumed=True,
        payload_deref_checked=True,
        payload_deref_rejected=True,
        payload_deref_allowed_for_resident_payload=False,
        payload_deref_attempted=False,
        payload_handle_deref_checked=True,
        payload_handle_deref_rejected=True,
        ...
        real_payload_ready=False,
        copy_completed=False,
        request_source=str(residency.request_source),
        request_layer_idx=int(residency.request_layer_idx),
        request_expert_idx=int(residency.request_expert_idx),
        requested_payload_bytes=int(residency.requested_payload_bytes),
        source_issue_packet_count=int(residency.source_issue_packet_count),
        source_issue_unique_key_count=int(residency.source_issue_unique_key_count),
